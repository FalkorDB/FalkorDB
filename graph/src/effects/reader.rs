...
impl<'a> Reader<'a> {
    ...
    ///
    /// [`Self::take_n`] passes the item's exact width and gets back a count it
    /// can read in one go. The variable-width callers — a loop over strings,
    /// values or segments, where the count is known before the items' sizes
    /// are — pass the smallest an item can be, which caps the loop without
    /// predicting the payload. Both are the same question.
    ///
    /// The bound has to be here rather than inside the reader because the
    /// allocation is: every caller follows this with a `with_capacity`, and a
    /// count taken off the wire sizes it. Unguarded, twelve bytes of input ask
    /// for 64 GiB — see `blocks::tests`. `min_bytes_each` belongs to the type
    /// being decoded, so each one is a named constant on that type rather than
    /// a number invented at the call site.
    ...
    pub fn take_n<const W: usize, T>(
        ...
        // The same check `guard_count` makes, because it is the same question:
        // W is this item's exact width rather than a lower bound, so the answer
        // is also the byte count to read.
        let n = self.guard_count(count, W)?;
