...
//!
//! Measured at the data-structure boundary the gap was 1,990 against 1,011 at
//! `k = 1` and 4,810 against 1,160 at `k = 2`. This decomposes it, because the
//! two obvious explanations imply different fixes: if the cost is *materialising
//! identifiers* then not materialising them is the fix, and if it is *attaching
//! a `GxB_Iterator` per call* then no amount of counting-instead-of-collecting
//! will move it.
//!
//! **It was mostly the second, and that was not the expectation.** At `k = 1`
//! counting the identifiers costs 81 instructions of the 1,990; the row scan
//! underneath costs 1,908, of which 1,608 is getting an iterator (the same scan
//! through one that is re-seeked costs 300). So the dominant term is per-call
//! iterator setup, and a degree that merely stops collecting identifiers would
//! have moved almost nothing.
//!
//! Hence two changes rather than one: pooling `GxB_Iterator` handles, which is
//! general and helps every row scan in the engine, and then `row_degree` /
//! `col_degree`, which drop the per-pair `Vec` and share one `me` cursor across
//! a row. The rows below show both, so the split stays visible.
//!
//! Run with:
...
fn degree_decomposition() {
    ...
    for k in [1u64, 2] {
        ...
        // `k = 1` this reads nothing and prices the wasted attach alone.
        for _ in 0..3 {
            ...
            let c = per_op(OPS, || {
                let mut n = 0usize;
                for r in 0..OPS {
                    let key = super::tensor::compound_key(r % PAIRS, r % PAIRS);
                    it.seek(key, key);
                    n += it.by_ref().count();
                }
                std::hint::black_box(n);
