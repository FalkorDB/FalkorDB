...
pub struct Pending {
    ...
    /// `cancel_node_id` hands its id straight back to the recycle bin, so the
    ...
    /// `cancel_relationship_id` puts them in the recycle bin — so a buffer that
...
impl Pending {
    ...
    pub fn commit(
        ...
        if !self.created_nodes.is_empty() {
            ...
            g.borrow_mut()
                .create_nodes(&self.created_nodes)
