...
use graph::{
    effects::{EffectEncode, EffectsBuffer, v3},
...
fn report_sizes() {
    ...
    for n in SCALES {
        ...
        buf.build(&p, &g);
        let plain = buf.as_slice().len();
        // Compression works on the raw bytes, so take a copy rather than ask
        // the buffer to compress itself — sealing it is the point.
        let mut z = buf.as_slice().to_vec();
...
fn decode(c: &mut Criterion) {
    ...
    for n in SCALES {
        let (g, p) = fixture(n);
        let mut plain = EffectsBuffer::new();
        plain.build(&p, &g);
        let plain = plain.as_slice().to_vec();
        ...
        group.bench_with_input(BenchmarkId::new("v3", n), &n, |b, _| {
            b.iter(|| {
                black_box(
                    v3::open_payload(black_box(&plain))
                        .unwrap()
                        .records()
                        .count(),
                )
        ...
        group.bench_with_input(BenchmarkId::new("v3+zstd-1", n), &n, |b, _| {
            b.iter(|| {
                black_box(
                    v3::open_payload(black_box(&compressed))
                        .unwrap()
                        .records()
                        .count(),
                )
    ...
    for n in [1_000_usize, 10_000, 100_000] {
        ...
        for (shape, buf) in [("1shape", &one), ("Nshapes", &many)] {
            ...
            group.bench_with_input(
                ...
                |b, buf| {
                    b.iter(|| {
                        black_box(v3::open_payload(black_box(buf)).unwrap().records().count())
                    });
                },
            );
            group.bench_with_input(
...
fn writer_breakdown(c: &mut Criterion) {
    ...
    group.bench_function("attr_block", |b| {
        b.iter(|| {
            let mut buf = Vec::new();
            v3::AttrIds(&[0_u16, 1][..]).encode(&mut buf);
            v3::AttrValues(black_box(rows.as_slice())).encode(&mut buf);
            black_box(buf.as_slice().len())
        });
    });
