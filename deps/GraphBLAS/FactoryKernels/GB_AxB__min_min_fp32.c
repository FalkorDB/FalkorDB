//------------------------------------------------------------------------------
// GB_AxB__min_min_fp32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) z = fminf (z, fminf (a,b))
#define GB_MULT(z,a,b,i,k,j)    z = fminf (a,b)
...
#if (defined(GxB_NO_MIN) || defined(GxB_NO_FP32) || defined(GxB_NO_MIN_FP32) || defined(GxB_NO_MIN_MIN_FP32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__min_min_fp32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__min_min_fp32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__min_min_fp32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
