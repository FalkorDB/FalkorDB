//------------------------------------------------------------------------------
// GB_AxB__plus_land_int32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { int32_t x_op_y = ((a != 0) && (b != 0)) ; z += x_op_y ; }
...
#if (defined(GxB_NO_PLUS) || defined(GxB_NO_LAND) || defined(GxB_NO_INT32) || defined(GxB_NO_PLUS_INT32) || defined(GxB_NO_LAND_INT32) || defined(GxB_NO_PLUS_LAND_INT32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__plus_land_int32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__plus_land_int32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__plus_land_int32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
