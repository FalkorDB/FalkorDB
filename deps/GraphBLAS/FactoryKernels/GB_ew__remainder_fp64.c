...
// operator:
#define GB_BINOP(z,x,y,i,j) z = remainder (x, y)
...
#if (defined(GxB_NO_REMAINDER) || defined(GxB_NO_FP64) || defined(GxB_NO_REMAINDER_FP64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulln__remainder_fp64)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__remainder_fp64)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
