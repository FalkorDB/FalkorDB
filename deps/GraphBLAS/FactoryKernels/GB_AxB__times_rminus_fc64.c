//------------------------------------------------------------------------------
// GB_AxB__times_rminus_fc64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { GxB_FC64_t x_op_y = GB_FC64_minus (b,a) ; z = GB_FC64_mul (z, x_op_y) ; }
#define GB_MULT(z,a,b,i,k,j)    z = GB_FC64_minus (b,a)
#define GB_ADD(z,zin,t)         z = GB_FC64_mul (zin, t)
#define GB_UPDATE(z,t)          z = GB_FC64_mul (z, t)
...
#define GB_DECLAREA(aik) GxB_FC64_t aik
...
#define GB_DECLAREB(bkj) GxB_FC64_t bkj
...
#if (defined(GxB_NO_TIMES) || defined(GxB_NO_RMINUS) || defined(GxB_NO_FC64) || defined(GxB_NO_TIMES_FC64) || defined(GxB_NO_RMINUS_FC64) || defined(GxB_NO_TIMES_RMINUS_FC64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__times_rminus_fc64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__times_rminus_fc64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__times_rminus_fc64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
