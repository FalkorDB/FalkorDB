//------------------------------------------------------------------------------
// GB_AxB__max_rdiv_int64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { int64_t x_op_y = GB_idiv_int64 (b, a) ; z = GB_IMAX (z, x_op_y) ; }
#define GB_MULT(z,a,b,i,k,j)    z = GB_idiv_int64 (b, a)
...
#define GB_DECLAREA(aik) int64_t aik
...
#define GB_DECLAREB(bkj) int64_t bkj
...
#define GB_DECLARE_IDENTITY_CONST(z) const int64_t z = INT64_MIN
...
#define GB_DECLARE_TERMINAL_CONST(zterminal) const int64_t zterminal = INT64_MAX

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_MAX) || defined(GxB_NO_RDIV) || defined(GxB_NO_INT64) || defined(GxB_NO_MAX_INT64) || defined(GxB_NO_RDIV_INT64) || defined(GxB_NO_MAX_RDIV_INT64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__max_rdiv_int64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__max_rdiv_int64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__max_rdiv_int64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
