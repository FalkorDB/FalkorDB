//------------------------------------------------------------------------------
// GB_AxB__max_min_int64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { int64_t x_op_y = GB_IMIN (a,b) ; z = GB_IMAX (z, x_op_y) ; }
...
#if (defined(GxB_NO_MAX) || defined(GxB_NO_MIN) || defined(GxB_NO_INT64) || defined(GxB_NO_MAX_INT64) || defined(GxB_NO_MIN_INT64) || defined(GxB_NO_MAX_MIN_INT64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__max_min_int64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__max_min_int64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__max_min_int64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
