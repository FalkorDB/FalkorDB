//------------------------------------------------------------------------------
// GB_AxB__min_times_uint8.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint8_t x_op_y = (a*b) ; z = GB_IMIN (z, x_op_y) ; }
...
#if (defined(GxB_NO_MIN) || defined(GxB_NO_TIMES) || defined(GxB_NO_UINT8) || defined(GxB_NO_MIN_UINT8) || defined(GxB_NO_TIMES_UINT8) || defined(GxB_NO_MIN_TIMES_UINT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__min_times_uint8) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__min_times_uint8) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__min_times_uint8) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
