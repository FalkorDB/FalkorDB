//------------------------------------------------------------------------------
// GB_AxB__min_div_uint16.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint16_t x_op_y = GB_idiv_uint16 (a,b) ; z = GB_IMIN (z, x_op_y) ; }
...
#if (defined(GxB_NO_MIN) || defined(GxB_NO_DIV) || defined(GxB_NO_UINT16) || defined(GxB_NO_MIN_UINT16) || defined(GxB_NO_DIV_UINT16) || defined(GxB_NO_MIN_DIV_UINT16))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__min_div_uint16) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__min_div_uint16) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__min_div_uint16) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
