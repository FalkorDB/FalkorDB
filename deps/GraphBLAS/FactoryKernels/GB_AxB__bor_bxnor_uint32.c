//------------------------------------------------------------------------------
// GB_AxB__bor_bxnor_uint32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
#define GB_TERMINAL_CONDITION(z,zterminal) (z == 0xFFFFFFFF)
#define GB_IF_TERMINAL_BREAK(z,zterminal) if (z == 0xFFFFFFFF) { break ; }
#define GB_DECLARE_TERMINAL_CONST(zterminal) const uint32_t zterminal = 0xFFFFFFFF

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_BOR) || defined(GxB_NO_BXNOR) || defined(GxB_NO_UINT32) || defined(GxB_NO_BOR_UINT32) || defined(GxB_NO_BXNOR_UINT32) || defined(GxB_NO_BOR_BXNOR_UINT32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__bor_bxnor_uint32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__bor_bxnor_uint32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__bor_bxnor_uint32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
