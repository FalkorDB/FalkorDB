//------------------------------------------------------------------------------
// GB_AxB__min_second_uint32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint32_t x_op_y = b ; z = GB_IMIN (z, x_op_y) ; }
...
#if (defined(GxB_NO_MIN) || defined(GxB_NO_SECOND) || defined(GxB_NO_UINT32) || defined(GxB_NO_MIN_UINT32) || defined(GxB_NO_SECOND_UINT32) || defined(GxB_NO_MIN_SECOND_UINT32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__min_second_uint32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__min_second_uint32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__min_second_uint32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
