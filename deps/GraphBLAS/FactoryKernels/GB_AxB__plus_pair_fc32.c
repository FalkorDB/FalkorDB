//------------------------------------------------------------------------------
// GB_AxB__plus_pair_fc32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) z = GB_FC32_add (z, GxB_CMPLXF(1,0))
#define GB_MULT(z,a,b,i,k,j)    z = GxB_CMPLXF(1,0)
...
#if (defined(GxB_NO_PLUS) || defined(GxB_NO_PAIR) || defined(GxB_NO_FC32) || defined(GxB_NO_PLUS_FC32) || defined(GxB_NO_PAIR_FC32) || defined(GxB_NO_PLUS_PAIR_FC32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__plus_pair_fc32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__plus_pair_fc32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__plus_pair_fc32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
