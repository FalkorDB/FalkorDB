//------------------------------------------------------------------------------
// GB_AxB__max_rdiv_int8.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { int8_t x_op_y = GB_idiv_int8 (b, a) ; z = GB_IMAX (z, x_op_y) ; }
#define GB_MULT(z,a,b,i,k,j)    z = GB_idiv_int8 (b, a)
...
#define GB_DECLARE_IDENTITY(z) int8_t z = INT8_MIN
#define GB_DECLARE_IDENTITY_CONST(z) const int8_t z = INT8_MIN
#define GB_Z_NBITS 8
#define GB_HAS_IDENTITY_BYTE 1
#define GB_IDENTITY_BYTE 0x80
...
#define GB_TERMINAL_CONDITION(z,zterminal) (z == INT8_MAX)
#define GB_IF_TERMINAL_BREAK(z,zterminal) if (z == INT8_MAX) { break ; }
#define GB_DECLARE_TERMINAL_CONST(zterminal) const int8_t zterminal = INT8_MAX

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_MAX) || defined(GxB_NO_RDIV) || defined(GxB_NO_INT8) || defined(GxB_NO_MAX_INT8) || defined(GxB_NO_RDIV_INT8) || defined(GxB_NO_MAX_RDIV_INT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__max_rdiv_int8) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__max_rdiv_int8) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__max_rdiv_int8) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
