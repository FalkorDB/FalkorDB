...
// operator:
#define GB_BINOP(z,x,y,i,j) z = GB_bitget_int8 (x, y)
...
#if (defined(GxB_NO_BGET) || defined(GxB_NO_INT8) || defined(GxB_NO_BGET_INT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulln__bget_int8)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__bget_int8)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
