//------------------------------------------------------------------------------
// GB_AxB__land_lxor_bool.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
#if (defined(GxB_NO_LAND) || defined(GxB_NO_LXOR) || defined(GxB_NO_BOOL) || defined(GxB_NO_LAND_BOOL) || defined(GxB_NO_LXOR_BOOL) || defined(GxB_NO_LAND_LXOR_BOOL))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__land_lxor_bool) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__land_lxor_bool) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__land_lxor_bool) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
