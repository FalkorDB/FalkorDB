//------------------------------------------------------------------------------
// GB_AxB__bxnor_band_uint8.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint8_t x_op_y = (a & b) ; z = ~(z ^ x_op_y) ; }
...
#define GB_DECLARE_IDENTITY(z) uint8_t z = 0xFF
#define GB_DECLARE_IDENTITY_CONST(z) const uint8_t z = 0xFF
#define GB_Z_NBITS 8
...
#if (defined(GxB_NO_BXNOR) || defined(GxB_NO_BAND) || defined(GxB_NO_UINT8) || defined(GxB_NO_BXNOR_UINT8) || defined(GxB_NO_BAND_UINT8) || defined(GxB_NO_BXNOR_BAND_UINT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__bxnor_band_uint8) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__bxnor_band_uint8) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__bxnor_band_uint8) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
