//------------------------------------------------------------------------------
// GB_AxB__min_second_fp64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) z = fmin (z, b)
#define GB_MULT(z,a,b,i,k,j)    z = b
#define GB_ADD(z,zin,t)         z = fmin (zin, t)
...
#if (defined(GxB_NO_MIN) || defined(GxB_NO_SECOND) || defined(GxB_NO_FP64) || defined(GxB_NO_MIN_FP64) || defined(GxB_NO_SECOND_FP64) || defined(GxB_NO_MIN_SECOND_FP64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__min_second_fp64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__min_second_fp64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__min_second_fp64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
