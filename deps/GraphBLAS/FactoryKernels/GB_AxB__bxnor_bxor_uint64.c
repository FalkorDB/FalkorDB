//------------------------------------------------------------------------------
// GB_AxB__bxnor_bxor_uint64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint64_t x_op_y = (a ^ b) ; z = ~(z ^ x_op_y) ; }
...
#define GB_DECLARE_IDENTITY(z) uint64_t z = 0xFFFFFFFFFFFFFFFFL
#define GB_DECLARE_IDENTITY_CONST(z) const uint64_t z = 0xFFFFFFFFFFFFFFFFL
...
#if (defined(GxB_NO_BXNOR) || defined(GxB_NO_BXOR) || defined(GxB_NO_UINT64) || defined(GxB_NO_BXNOR_UINT64) || defined(GxB_NO_BXOR_UINT64) || defined(GxB_NO_BXNOR_BXOR_UINT64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__bxnor_bxor_uint64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__bxnor_bxor_uint64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__bxnor_bxor_uint64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
