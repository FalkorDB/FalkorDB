...
// operator:
#define GB_BINOP(z,x,y,i,j) z = GB_bitclr_uint64 (x, y)
...
#if (defined(GxB_NO_BCLR) || defined(GxB_NO_UINT64) || defined(GxB_NO_BCLR_UINT64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulln__bclr_uint64)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__bclr_uint64)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
