//------------------------------------------------------------------------------
// GB_AxB__plus_min_uint64.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint64_t x_op_y = GB_IMIN (a,b) ; z += x_op_y ; }
...
#if (defined(GxB_NO_PLUS) || defined(GxB_NO_MIN) || defined(GxB_NO_UINT64) || defined(GxB_NO_PLUS_UINT64) || defined(GxB_NO_MIN_UINT64) || defined(GxB_NO_PLUS_MIN_UINT64))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__plus_min_uint64) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__plus_min_uint64) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__plus_min_uint64) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
