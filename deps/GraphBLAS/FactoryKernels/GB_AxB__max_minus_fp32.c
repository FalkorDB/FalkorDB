//------------------------------------------------------------------------------
// GB_AxB__max_minus_fp32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) z = fmaxf (z, (a-b))
...
#if (defined(GxB_NO_MAX) || defined(GxB_NO_MINUS) || defined(GxB_NO_FP32) || defined(GxB_NO_MAX_FP32) || defined(GxB_NO_MINUS_FP32) || defined(GxB_NO_MAX_MINUS_FP32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__max_minus_fp32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__max_minus_fp32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__max_minus_fp32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
