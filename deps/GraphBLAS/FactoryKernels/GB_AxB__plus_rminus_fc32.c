//------------------------------------------------------------------------------
// GB_AxB__plus_rminus_fc32.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { GxB_FC32_t x_op_y = GB_FC32_minus (b,a) ; z = GB_FC32_add (z, x_op_y) ; }
#define GB_MULT(z,a,b,i,k,j)    z = GB_FC32_minus (b,a)
#define GB_ADD(z,zin,t)         z = GB_FC32_add (zin, t)
...
#if (defined(GxB_NO_PLUS) || defined(GxB_NO_RMINUS) || defined(GxB_NO_FC32) || defined(GxB_NO_PLUS_FC32) || defined(GxB_NO_RMINUS_FC32) || defined(GxB_NO_PLUS_RMINUS_FC32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__plus_rminus_fc32) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__plus_rminus_fc32) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__plus_rminus_fc32) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
