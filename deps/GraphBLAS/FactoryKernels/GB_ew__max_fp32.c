...
// operator:
#define GB_BINOP(z,x,y,i,j) z = fmaxf (x, y)
...
#if (defined(GxB_NO_MAX) || defined(GxB_NO_FP32) || defined(GxB_NO_MAX_FP32))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
GrB_Info GB (_Cewise_fulla__max_fp32)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_Cewise_fulln__max_fp32)
(
    GrB_Matrix C,
    const GrB_Matrix A,
    const GrB_Matrix B,
...
GrB_Info GB (_AunionB__max_fp32)
(
    GrB_Matrix C,
    const int C_sparsity,
    const GrB_Matrix M,
