//------------------------------------------------------------------------------
// GB_AxB__times_rdiv_int8.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { int8_t x_op_y = GB_idiv_int8 (b, a) ; z *= x_op_y ; }
...
#define GB_DECLARE_IDENTITY(z) int8_t z = 1
#define GB_DECLARE_IDENTITY_CONST(z) const int8_t z = 1
#define GB_Z_NBITS 8
...
#define GB_DECLARE_TERMINAL_CONST(zterminal) const int8_t zterminal = 0

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_TIMES) || defined(GxB_NO_RDIV) || defined(GxB_NO_INT8) || defined(GxB_NO_TIMES_INT8) || defined(GxB_NO_RDIV_INT8) || defined(GxB_NO_TIMES_RDIV_INT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__times_rdiv_int8) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__times_rdiv_int8) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__times_rdiv_int8) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
