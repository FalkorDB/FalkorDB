//------------------------------------------------------------------------------
// GB_AxB__bor_bxnor_uint8.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint8_t x_op_y = ~(a ^ b) ; z |= x_op_y ; }
...
#define GB_TERMINAL_CONDITION(z,zterminal) (z == 0xFF)
#define GB_IF_TERMINAL_BREAK(z,zterminal) if (z == 0xFF) { break ; }
#define GB_DECLARE_TERMINAL_CONST(zterminal) const uint8_t zterminal = 0xFF

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_BOR) || defined(GxB_NO_BXNOR) || defined(GxB_NO_UINT8) || defined(GxB_NO_BOR_UINT8) || defined(GxB_NO_BXNOR_UINT8) || defined(GxB_NO_BOR_BXNOR_UINT8))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__bor_bxnor_uint8) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__bor_bxnor_uint8) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__bor_bxnor_uint8) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
