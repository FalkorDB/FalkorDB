//------------------------------------------------------------------------------
// GB_AxB__max_rdiv_uint16.c: matrix multiply for a single semiring
//------------------------------------------------------------------------------
...
// semiring operators:
#define GB_MULTADD(z,a,b,i,k,j) { uint16_t x_op_y = GB_idiv_uint16 (b, a) ; z = GB_IMAX (z, x_op_y) ; }
#define GB_MULT(z,a,b,i,k,j)    z = GB_idiv_uint16 (b, a)
...
#define GB_TERMINAL_CONDITION(z,zterminal) (z == UINT16_MAX)
#define GB_IF_TERMINAL_BREAK(z,zterminal) if (z == UINT16_MAX) { break ; }
#define GB_DECLARE_TERMINAL_CONST(zterminal) const uint16_t zterminal = UINT16_MAX

// special case multipliers:

// disable this semiring and use the generic case if these conditions hold
#if (defined(GxB_NO_MAX) || defined(GxB_NO_RDIV) || defined(GxB_NO_UINT16) || defined(GxB_NO_MAX_UINT16) || defined(GxB_NO_RDIV_UINT16) || defined(GxB_NO_MAX_RDIV_UINT16))
#define GB_DISABLE 1
#else
#define GB_DISABLE 0
#endif
...
{ 
    ...
    {
        // C = A*B, no mask
        return (GB (_Asaxpy3B_noM__max_rdiv_uint16) (C, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    else if (!Mask_comp)
    {
        // C<M> = A*B
        return (GB (_Asaxpy3B_M__max_rdiv_uint16) (C,
    ...
    {
        // C<!M> = A*B
        return (GB (_Asaxpy3B_notM__max_rdiv_uint16) (C,
            M, Mask_struct, M_in_place, A, B,
            SaxpyTasks, ntasks, nfine, nthreads, do_sort, Werk)) ;
    }
    #endif
