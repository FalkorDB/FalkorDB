//------------------------------------------------------------------------------
// GB_jit__select_phase2__3ff318e85__LG_MSF_selectEdge_int64_t.c
//------------------------------------------------------------------------------
...
// op: LG_MSF_selectEdge_int64_t (flipped ij), ztype: bool, xtype: int64_t, ytype: LG_MSF_context_int64_t

typedef struct { uint64_t *parent; struct { int64_t wInt; uint64_t idx; } *w_partner; } LG_MSF_context_int64_t;
#define GB_LG_MSF_context_int64_t_USER_DEFN \
"typedef struct { uint64_t *parent; struct { int64_t wInt; uint64_t idx; } *w_partner; } LG_MSF_context_int64_t;"

// unary operator types:
#define GB_Z_TYPE bool
#define GB_X_TYPE int64_t
#define GB_Y_TYPE LG_MSF_context_int64_t

// index unary operator (flipped ij):
#ifndef GB_GUARD_LG_MSF_selectEdge_int64_t_DEFINED
#define GB_GUARD_LG_MSF_selectEdge_int64_t_DEFINED
GB_STATIC_INLINE
void LG_MSF_selectEdge_int64_t ( _Bool *z, const int64_t *x, GrB_Index i, GrB_Index j, const LG_MSF_context_int64_t *theta ) { (*z) = (theta->w_partner[i].wInt == *x) && (theta->parent[j] == theta->w_partner[i].idx); }
#define GB_LG_MSF_selectEdge_int64_t_USER_DEFN \
"void LG_MSF_selectEdge_int64_t ( _Bool *z, const int64_t *x, GrB_Index i, GrB_Index j, const LG_MSF_context_int64_t *theta ) { (*z) = (theta->w_partner[i].wInt == *x) && (theta->parent[j] == theta->w_partner[i].idx); }"
#endif
#define GB_IDXUNOP(z,x,j,i,y) LG_MSF_selectEdge_int64_t (&(z), &(x), i, j, &(y))
...
#define GB_jit_kernel GB_jit__select_phase2__3ff318e85__LG_MSF_selectEdge_int64_t
#define GB_jit_query  GB_jit__select_phase2__3ff318e85__LG_MSF_selectEdge_int64_t_query
#endif
#include "template/GB_jit_kernel_select_phase2.c"
GB_JIT_GLOBAL GB_JIT_QUERY_PROTO (GB_jit_query) ;
GB_JIT_GLOBAL GB_JIT_QUERY_PROTO (GB_jit_query)
{
    (*hash) = 0x3c86aedc8cc64912 ;
    v [0] = 10 ; v [1] = 3 ; v [2] = 1 ;
    defn [0] = GB_LG_MSF_selectEdge_int64_t_USER_DEFN ;
    defn [1] = NULL ;
    defn [2] = NULL ;
    defn [3] = NULL ;
    defn [4] = NULL ;
