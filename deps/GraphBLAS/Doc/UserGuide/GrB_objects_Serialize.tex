...
created by v9.0.0; they simply ignore those components.  SS:GraphBLAS v10 adds
32/64-bit integers, and can read the blobs created by any prior version of
GraphBLAS (they be deserialized with all 64-bit integers however).  If an older
version of SS:GraphBLAS (v9 or earlier) attempts to deserialize a blob
containing a matrix with 32-bit integers, it will safely report that the blob
is invalid and refuse to deserialize it.  If SS:GraphBLAS v10 creates a
serialized blob with all-64-bit integers, then it can be read correctly by
SS:GraphBLAS v9, and likely also be earlier versions.
