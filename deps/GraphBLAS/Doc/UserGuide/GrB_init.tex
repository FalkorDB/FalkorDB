...
\verb'NULL'.  If \verb'user_realloc_func' is \verb'NULL', then
\verb'user_malloc_func' is relied on instead.

These functions can only be set once, when GraphBLAS starts.  They can be
queried using \verb'GrB_get' (see Section~\ref{get_set_global}).  Either
...
thread-safe.  The following usage is identical to \verb'GrB_init(mode)':
...
GraphBLAS itself does not use the system \verb'calloc' nor the
\verb'user_calloc_func', except the function pointer is returned by the
\verb'GrB_get' method via the \verb'GxB_CALLOC_FUNCTION' enum;
see Sections~\ref{get_set_enums} and \ref{get_set_global}.
