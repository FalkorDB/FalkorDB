...
The `me` half is where `compound_key` is inverted: each entry's `(block, row)`
address is reassembled into `(src, dst)` by `compound_key_inverse`. Since #2579
that inversion is *total* — `inv_key` holds for every pair — where its
predecessor needed the pair to fit two `u32` halves. -/
theorem mem_iterEdges (h : Inv t) {x : Nat × Nat × Nat} :
    x ∈ iterEdges t ↔ x.2.2 ∈ t.edgesAt (x.1, x.2.1) := by
  simp only [iterEdges, Multiset.mem_add, Multiset.mem_filter, fwdIterAll, Multiset.mem_map,
    Finset.mem_val]
  constructor
  · rintro (⟨⟨p, hp, hx⟩, hne⟩ | ⟨y, hy, hxy⟩)
    · obtain ⟨v, hv⟩ := Option.isSome_iff_exists.mp (mem_effDom_iff_isSome.mp hp)
      ...
      simp
    · obtain ⟨q, hqdom, hqk⟩ := h.me_keyed y hy
      have hrow : y.2 ∈ t.meRow (key q) := mem_meRow.mpr (by rw [← hqk]; exact hy)
      have hmulti : t.effGet q = some MULTI := by
        by_contra hne
        rw [h.row_empty q hne] at hrow
        exact absurd hrow (by simp)
      have hinv : keyInverse y.1.1 y.1.2 = q := by rw [hqk]; exact inv_key q
      have h1 : x.1 = q.1 := by rw [← hxy]; simp [hinv]
      have h2 : x.2.1 = q.2 := by rw [← hxy]; simp [hinv]
  ...
  · intro hmem
    ...
    · subst hM
      ...
      · simp only [key]
        rw [inv_key (x.1, x.2.1)]
    · refine Or.inl ⟨⟨(x.1, x.2.1), hdom, ?_⟩, ?_⟩
      · have hxv : x.2.2 = v := by
...
theorem mem_iterEdges_fwd {x : Nat × Nat × Nat}
    (hx : x ∈ t.fwdIterAll.filter (fun y => y.2.2 ≠ MULTI)) :
    t.effGet (x.1, x.2.1) = some x.2.2 ∧ x.2.2 ≠ MULTI := by
  simp only [Multiset.mem_filter, fwdIterAll, Multiset.mem_map, Finset.mem_val] at hx
  obtain ⟨⟨p, hp, hxp⟩, hne⟩ := hx
  obtain ⟨v, hv⟩ := Option.isSome_iff_exists.mp (mem_effDom_iff_isSome.mp hp)
...
theorem mem_iterEdges_me (h : Inv t) {x : Nat × Nat × Nat}
    (hx : x ∈ t.me.val.map (fun y => let p := keyInverse y.1.1 y.1.2; (p.1, p.2, y.2))) :
...
theorem nodup_iterEdges (h : Inv t) : (iterEdges t).Nodup := by
  ...
  · refine Multiset.Nodup.filter _ ?_
    refine Multiset.Nodup.map_on ?_ t.effDom.nodup
    intro p _ q _ hpq
    exact Prod.ext (congrArg (fun z => z.1) hpq) (congrArg (fun z => z.2.1) hpq)
  · refine Multiset.Nodup.map_on ?_ t.me.nodup
    intro y hy z hz hyz
    obtain ⟨q, _, hqk⟩ := h.me_keyed y (Finset.mem_val.mp hy)
    obtain ⟨r, _, hrk⟩ := h.me_keyed z (Finset.mem_val.mp hz)
    have hq : keyInverse y.1.1 y.1.2 = q := by rw [hqk]; exact inv_key q
    have hr : keyInverse z.1.1 z.1.2 = r := by rw [hrk]; exact inv_key r
    have h1 : y.1 = z.1 := by
      have e1 : q.1 = r.1 := by simpa [hq, hr] using congrArg Prod.fst hyz
      have e2 : q.2 = r.2 := by simpa [hq, hr] using congrArg (fun w => w.2.1) hyz
      rw [hqk, hrk]
