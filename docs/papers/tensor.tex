...
\emph{all} of the pair's identifiers migrate to a shared hypersparse overflow
matrix keyed by a $(\text{src},\text{dst})$ compound key that names a block and a
row within it --- one matrix per block, which for any graph below $2^{30}$ nodes
per axis means one matrix.
...
\begin{enumerate}
  ...
  \item \textbf{A shared overflow matrix instead of per-cell
        containers} (\S\ref{sec:compound}). Multi-edge identifiers live in a
        hypersparse \cite{bulucgilbert2008} Boolean matrix per block --- one
        matrix for any graph below $2^{30}$ nodes per axis --- indexed by a
        \emph{compound key} $\kappa$ --- which cuts each endpoint in two,
        the low $30$ bits packing into the row and the high bits selecting a
        block --- in the row dimension, and by edge identifier in the column
        dimension. This
  ...
  \item \textbf{An explicit account of the design's \emph{domain
        restrictions}} (\S\ref{sec:limits}). The sharpest of them was a
        defect: the compound key's guard admitted node identifiers up to
        $2^{32}-1$ while the overflow matrix's dimension bounded the source
        identifier of a multi-edge pair by $2^{28}$, and in release builds the
        out-of-range write was dropped silently. We report the reproduction,
        the repair that removed the ceiling rather than documenting it, and
        the two places where what shipped differs from what this paper first
        proposed.
\end{enumerate}
...
of a shared matrix. Its distinguishing cost is the promotion/demotion
...
pub struct Tensor {
    ...
    me: SmallVec<[(MeBlock, VersionedMatrix<bool>); 1]>, // overflow, one matrix
                                 // per live block: row key -> edge id
...
\node[aux] (me) at (8.4,-2.7)  {$me$ \;\texttt{BOOL}, $2^{60}\!\times\!2^{31}$ per block\\\scriptsize $\kappa(src,dst) \to$ edge id};
...
not provide, the pair is folded into an index. It is folded into \emph{two}:
a block, and a row within that block,
\[
  \kappa(s,d) \;=\;
    \Bigl(\underbrace{(s \gg k,\; d \gg k)}_{\text{block}},\;
          \underbrace{((s \wedge m) \ll k) \mathbin{|} (d \wedge m)}_{\text{row}}\Bigr),
  \qquad k = 30,\ m = 2^{k}-1 ,
\]
and $me$ is a collection of matrices indexed by block, with
$me_{\text{block}}\bigl[\text{row},\, e\bigr] = \texttt{true}$. Equivalently,
the $s \times d$ identifier space is tiled into $2^{30} \times 2^{30}$ tiles:
the block names the tile, the row names the cell inside it.

A pair's identifiers are still exactly one \emph{row} of one matrix; the row
scan GraphBLAS provides yields them in ascending column order, i.e.\ in
ascending identifier order, for free. Because each block is hypersparse, its
$2^{60}$-row declaration costs nothing.

Three properties follow from building the row out of \emph{masked} halves, and
they are the reason for the shape.

\paragraph{It is total.} Two $k$-bit halves occupy $2k = 60$ bits, which is
exactly GraphBLAS's index budget, so the row is a valid index of $me$ for every
input. There is nothing to check and nothing to panic on:
...
Identifiers too large for one tile move to another rather than truncate. This
is the property the predecessor lacked, and \S\ref{sec:limits} tells that
story: $\kappa$ was $(s \ll 32) \mathbin{|} d$, which needed $60$ bits at
$s = 2^{28}$ and was then out of range, where the write was \emph{silently
dropped} in release builds.

\paragraph{It is invertible, and its injectivity is unconditional.} Each
endpoint is reassembled from its block coordinate and its half of the row, so
whole-tensor enumeration recovers $(s,d)$ without an external endpoint table.
Distinct pairs therefore never share an address --- with no hypothesis about
identifier width, which is what lets the mechanisation state
\texttt{key\_inj} outright (\S\ref{sec:mechanised}).

\paragraph{It costs a compare, not a lookup.} Every graph below $2^{30}$ nodes
per axis lands wholly in block $(0,0)$, and the tensor holds its blocks in a
list sized for exactly one, with block $(0,0)$ at index $0$. Such a graph pays
one comparison against the first entry --- no hashing, nothing on the heap ---
and has the layout the single-matrix design had. Maintenance paths (fold, wait,
resize) fold over \emph{live} blocks, so their cost is proportional to blocks
that exist rather than to the block index range.

The serialised form is unaffected: the blob stores $(s, d, \textit{ids})$ rather
than row keys, so it is block-agnostic already and re-keys on load. The packing
is an in-memory detail, never a format --- so the repair needed no version bump
and no migration.

\subsection{The overflow matrix is declared narrow}
\label{sec:narrow}

One further choice belongs here because it is where $me$'s space actually goes.
The row dimension is the full $2^{60}$, but the \emph{column} dimension is
declared at $2^{31}$, not $2^{60}$.

Columns of $me$ are edge identifiers, so the column-index array holds one entry
per stored identifier --- it is the largest array in the structure. GraphBLAS
chooses its index width per matrix from the declared dimensions, and stores
column indices in $32$ bits when the column count fits a \texttt{u32}. The
threshold is $2^{31}$ rather than $2^{32}$ because the dimension itself must
fit. Declaring the matrix $2^{60}$ wide forced $64$-bit indices for every
identifier; declaring it $2^{31}$ wide halves that array. Measured at $200{,}000$
identifiers, $6.44$ bytes per identifier against $11.69$ --- the declaration
alone accounted for $45\%$.

A tensor whose edge identifiers actually reach $2^{31}$ widens back and pays the
old price, so the narrow default costs only the check. Rows stay wide
deliberately: narrowing them saves a further $0.40$\,B/id --- row arrays hold
one entry per multi-edge \emph{pair}, not per identifier --- and would require
$k = 15$, i.e.\ a block per $32{,}768$ nodes per axis.

\S\ref{sec:evalspace} reports what this did to the measured space, including the
part of it that is a result rather than an erratum.
...
a flat scan of each live block of $me$, inverting $\kappa$ back into $(s,d)$
from the block a row was found in.
...
$\kappa$ is injective (\S\ref{sec:compound}) --- unconditionally, since it is
built from masked halves --- so the rows partition $me$.
\end{proof}
...
Compound key      & \ref{sec:compound}& \texttt{rowBits\_eq\_row} (bitwise $=$ arithmetic),
                                       \texttt{row\_le\_grbIndexMax} (always a writable
                                       index), \texttt{key\_inj}, \texttt{inv\_key}
                                       --- all with no hypothesis \\
...
\[
  ...
  \;+\; \underbrace{4\Bigl(\textstyle\sum_{p \in P_{\ge 2}} |\mathrm{Ids}(p)|\Bigr)}_{me:\ \text{32-bit column indices}}
...
designs. The coefficient on the $me$ term used to be $8$, and this paper argued
from it that the compound key was worth narrowing. The argument was right about
the quantity and wrong about which dimension to narrow. Cutting the key to
$30+30$ bits leaves the \emph{row} indices $64$-bit regardless, because $2^{60}$
still exceeds a \texttt{u32}; what the coefficient counts is the \emph{column}
index array, one entry per stored identifier. Declaring the column dimension at
$2^{31}$ (\S\ref{sec:narrow}) takes it to $4$, and \S\ref{sec:evalspace}
measures the effect.
...
\midrule
\textbf{total}          & $\approx16$\,B & $\approx12$\,B & $\approx16$\,B \\
\midrule
\multicolumn{4}{@{}l@{}}{\emph{and at $\mu > 1$, per multi-edge pair:}}\\
extra objects            & 1 container, heap & --- & --- \\
extra entries            & container payload & $4$\,B/edge in aux. & $4$\,B/edge in $me$ \\
wasted forward cell      & ---     & ---     & $12$\,B (holds $\M$) \\
\bottomrule
\end{tabular}
\caption{Per-edge space, $32$-bit forward/backward indices, and --- since
\S\ref{sec:narrow} --- $32$-bit column indices in the auxiliary matrix too. At
$\mu = 1$ the auxiliary entry is the only term that separates the designs, and
narrowing it moved (B) \emph{below} (A) and (C) on this idealised count. The
measured composition in \S\ref{sec:evalspace} does not follow the table there,
because it charges (B) the per-row overhead this table omits; the table counts
payload, the measurement counts the structure. Either way (C)'s advantage is not
raw bytes at unit multiplicity but
...
What selects the state is a resident delta on $me$. When the overflow matrix
carries \emph{any} pending entries, a multi-edge row read consults the delta
layer rather than reading the committed base alone, at a flat cost of about
$1{,}200$ instructions --- and flat is meant literally: one pending identifier
costs what eight thousand do. That accounts for all three of the puzzles at once.
The states are two because a delta is either empty or it is not. Size selects
between them because size decides where the fold policy last fired relative to
the final write. And the selection is non-monotonic because \emph{that} is not a
function of size.

The diagnosis is causal rather than circumstantial: adding a single pending
identifier to $me$, on a pair the probes never read, moves the measured read from
$2{,}851$ to $4{,}050$ instructions. Storage format, the hypersparse hyper-hash,
index widths and the forward deltas were each eliminated first, and the issue's
own recorded refutation of the lingering-delta hypothesis --- that an unrelated
write does not move the number --- turns out to be too weak a test, because the
fold policy is size-based and a delta of one entry never meets its threshold.

Two things follow, and the first is about method rather than about tensors. Our
own first attempt at this diagnosis concluded the opposite, that the cost was not
in edge storage at all, on the strength of a boundary fixture that read a flat
cost at every size. That fixture built its tensor in one batch --- which is
exactly the condition that leaves $me$'s delta empty and the read in the low
state. \emph{A fixture that cannot reproduce a bug will exonerate whatever it is
pointed at}, and the way out was to build the tensor as the engine does, by
accumulation, rather than to build what the engine holds.

The second is the paper's own stake. The lookup column of
Table~\ref{tab:evalread} \emph{is} a cost of this design, though not of
inline-first representation as such: it is the cost of reaching identifiers
through a second delta-layered matrix, which is the price (C) pays for keeping
them out of the adjacency cell. It is not an exotic corner --- both of the
largest sizes measured sit in the high state --- and it is fixable without
touching the representation. \S\ref{sec:future} says how.

\paragraph{This is the methodology caveat, earned twice, in opposite
directions.} \S\ref{sec:evalmethod} warns that an engine-level ratio is evidence
about engines, and the warning cashed out against us the first time: we had a
tensor-shaped explanation ready for a tensor-shaped number --- a second probe
into $me$ that a short row cannot amortise --- and a controlled measurement of
the read path refuted it. It then cashed out against us a second time and the
other way. The controlled measurement that did the refuting was itself
uncontrolled in a variable nobody had thought to hold: how the tensor was
\emph{built}. Built in one batch it reads flat; built by accumulation, as the
engine builds it, it reproduces both states. The first measurement was right that
the effect is not a per-identifier walk, and wrong that it is not edge storage.

The general lesson is not that engine-level numbers are untrustworthy and
boundary numbers are not. It is that a fixture is a claim about which variables
matter, and a fixture that cannot reproduce the phenomenon has already assumed
its conclusion. A reader is entitled to apply that discount to every number in
this section, which is why the insert result of \S\ref{sec:evalwrite} is stated
as a claim about an ordering rather than about a factor.
...
the metric we chose. Changing the order was only half the question, though ---
$200{,}000$ pairs is a working set that fits in cache whichever order it is read
in. \S\ref{sec:evalcold} changes the working set instead, and finds that the
blindness runs in \emph{both} directions.

\subsection{The remaining entry points}
\label{sec:evalentry}

Everything priced so far is a read or a bulk mutation. Three entry points the
host runtime also calls had not been measured on both sides, and one of them is
the worst ratio in this paper.
...
\toprule
 & (A) & (C) & (C)/(A) \\
\midrule
row degree, $k=1$              & $1{,}011$ & $1{,}979$ & $1.96$ \\
row degree, $k=2$              & $1{,}160$ & $4{,}337$ & $3.74$ \\
column degree, $k=1$           & $1{,}432$ & $2{,}575$ & $1.80$ \\
column degree, $k=2$           & $1{,}581$ & $4{,}933$ & $3.12$ \\
\addlinespace
bulk delete, all-inline (per pair) & $1{,}121$ & $1{,}279$ & $1.14$ \\
bulk insert, all-inline (per edge) & $1{,}487$ & $1{,}737$ & $1.17$ \\
\bottomrule
\end{tabular}
\caption{Instructions per operation at the data-structure boundary, same
method as Table~\ref{tab:evalfanout}. The degree rows are (C) as it now stands,
with the counting entry point of \#2572; the paths they replaced measured
$2{,}065$ and $4{,}926$ (row), $2{,}664$ and $5{,}523$ (column), so the entry
point bought $4\%$ at $k=1$ and $12\%$ at $k=2$. Per-operation counts are
insensitive to fixture size here --- the superseded path measures within $0.2\%$
at $100$k and $200$k pairs. The bulk paths are within $17\%$; degree is not, and
\S\ref{sec:evalentry} takes apart why, because the obvious reason is the wrong
one.}
\label{tab:evalentry}
\end{table}

\paragraph{Degree looked like the one place (C) pays for a missing entry point
rather than for its representation.} (A) implements degree as a scan of the row's
\emph{cells}: a scalar cell contributes one, and a tagged cell contributes
\texttt{GrB\_Vector\_nvals} of the container it points at --- a constant-time
read of a length that is already stored. Its cost is therefore proportional to
the number of pairs in the row and independent of $k$, which
Table~\ref{tab:evalentry} shows directly ($1{,}011 \to 1{,}160$ from $k=1$ to
$k=2$, a $15\%$ rise for twice the edges).

(C), when this was first measured, had no degree entry point at all. Degree was
computed one level up, in \texttt{Graph::get\_node\_outdegree}, by counting a
full single-row iteration --- which materialises every identifier in the row only
to discard it. That reads like the whole explanation, and the ratio's growth with
$k$ ($1.96\times$ at $k=1$, $4.21\times$ at $k=2$) reads like confirmation of
it.

\paragraph{It is not the explanation, and taking it apart is what found the real
one.} Decomposing the $k=1$ figure: of $1{,}990$ instructions, the forward
single-row scan underneath accounts for $1{,}908$, so \emph{materialising the
identifiers costs $81$}. And the same scan through an iterator that is
\emph{re-seeked} rather than rebuilt costs $300$. The dominant term --- $1{,}608$
of $1{,}990$, four fifths of the whole --- is neither the representation nor the
missing entry point. It is the cost of constructing a \texttt{GxB\_Iterator} per
call, which (A) pays once per degree and (C) pays once per row scan and again per
multi-edge pair.

The decomposition reproduces on the current tree and the proportion has grown,
because the entry point removed the only term it could reach: \texttt{row\_degree}
costs $1{,}979$, the bare row scan under it $1{,}951$ --- so counting the
identifiers now costs about $28$ --- and that scan through a re-seeked iterator
costs $298$. Per-call construction is therefore $1{,}653$ of $1{,}979$, five
sixths of the whole.

That is worth stating plainly because the first explanation was ours, it was
published in the previous revision of this table, and it would have led to a fix
that moved $81$ instructions. It also generalises past degree: the same per-call
construction sits under every point read in Table~\ref{tab:evalboundary}, which
is where (C)'s $3.89\times$ at $k=2$ mostly comes from.

\paragraph{One of the two has been addressed, and the decomposition is why the
other is worth doing.} \#2572 gave (C) the entry point it was missing:
\texttt{row\_degree}/\texttt{col\_degree} count rather than collect --- an inline
pair is one edge, a promoted pair is the length of its $me$ row --- and the row's
$me$ lookups share one cursor instead of attaching an iterator per pair. Measured
on the same fixture, $2{,}065 \to 1{,}979$ at $k=1$ and $4{,}926 \to 4{,}337$ at
$k=2$; transposed, $2{,}664 \to 2{,}575$ and $5{,}523 \to 4{,}933$. Four per cent
and twelve.

That is a small return for a real change, and the decomposition above is exactly
why it was bounded before it was written: the identifiers were never the cost.
A previous revision of this paper additionally reported recycling
\texttt{GxB\_Iterator} handles through a thread-local free list, with degree
falling to $1{,}203$ and $2{,}793$. \textbf{That change is not in the tree and
those figures do not reproduce}; the numbers in
Table~\ref{tab:evalentry} are what the current engine measures, and the earlier
ones described a prototype that was never committed. Recycling remains the
outstanding item, and it is the larger one: per-call construction is five sixths
of a degree call, and the same construction sits under every point read in
Table~\ref{tab:evalboundary}. Removing it needs a reusable cursor threaded
through the hot callers rather than a handle cache, which is an API change
(\S\ref{sec:future}).

We keep the superseded figures beside the current ones rather than silently
restating the better ones: the point of the decomposition is that a plausible
explanation survived a whole revision because nothing had asked it to account for
the total, and the same discipline applies to a number whose change never landed.

Degree's blast radius is bounded and worth stating so the original number is not
read as worse than it was: these four functions back Cypher's \texttt{indegree}
and \texttt{outdegree} built-ins and nothing else. Traversal does not route
through them. The iterator-construction cost underneath them, on the other hand,
is on every read path in the engine, which is why the smaller of the two findings
turned out to be the more valuable one.

\subsection{Residency: the same reads out of cache}
\label{sec:evalcold}

\S\ref{sec:limits} warns that every read in this paper is warm, and the
access-order measurement above could not lift that warning: it changed the order
in which $200{,}000$ pairs were read, not whether they fit in cache. This sweeps
the working set from far inside the cache to far outside it --- $10^4$ to
$8 \times 10^6$ pairs, $0.2$ to $558$\,MB resident --- probing in a scrambled
order at every size, on both engines, with the same fixture and drain semantics.
Because this is the one measurement here whose result \emph{is} a time, it is
reported as the minimum of three runs rather than the median: the host is not
quiet (\S\ref{sec:limits}), so every sample is an upper bound and the minimum is
the least contaminated one available. The absolute nanoseconds should be read as
upper bounds; the ratios, taken between two engines measured back to back under
the same contention, are the claim.

The instruction column is the control rather than the result. A cache miss
retires no extra instruction, so if the sweep is changing residency and not work,
instructions must stay flat. They do: (A) varies by under $0.04\%$ across the
whole range at both $k$, and (C) by $0.9\%$ at $k=1$ and $3.6\%$ at $k=2$ ---
the latter being $me$'s hypersparse row search deepening as it grows to
$1.6\times 10^7$ entries. The nanoseconds are the measurement.
...
\begin{tabular}{@{}lrrrcrrr@{}}
\toprule
 & \multicolumn{3}{c}{$k=1$, ns per read} & & \multicolumn{3}{c}{$k=2$, ns per read} \\
\cmidrule(lr){2-4}\cmidrule(lr){6-8}
pairs & (A) & (C) & (C)/(A) & & (A) & (C) & (C)/(A) \\
\midrule
$10^4$              & $21.9$  & $34.3$  & $1.57$ & & $40.4$  & $188.8$ & $4.67$ \\
$10^5$              & $23.0$  & $29.9$  & $1.30$ & & $83.8$  & $219.8$ & $2.62$ \\
$5\times10^5$       & $24.5$  & $30.7$  & $1.25$ & & $246.3$ & $274.4$ & $1.11$ \\
$2\times10^6$       & $81.2$  & $98.9$  & $1.22$ & & $360.6$ & $566.6$ & $1.57$ \\
$4\times10^6$       & $103.1$ & $166.7$ & $1.62$ & & $368.8$ & $651.9$ & $1.77$ \\
$8\times10^6$       & $117.7$ & $202.5$ & $1.72$ & & $391.4$ & $736.9$ & $1.88$ \\
\midrule
\multicolumn{8}{@{}l@{}}{\emph{instructions, flat across the sweep:}\quad
$k=1$: (A) $559$, (C) $640$--$646$ \quad
$k=2$: (A) $757$, (C) $2{,}905$--$3{,}008$}\\
\bottomrule
\end{tabular}
\caption{The same point read as $k$ is held fixed and the working set grows past
the cache. At $k=2$ the instruction metric says (C) costs $3.8$--$4.0\times$ (A)
at every size; time says $4.67\times$ in cache and $1.88\times$ out of it.}
\label{tab:evalcold}
\end{table}

\paragraph{At $k=2$ the instruction count overstates (C)'s disadvantage, by about
twofold at the largest size measured.} Instructions put (C) at $3.8$--$4.0\times$
(A) and put it there at every working-set size. Time agrees only in cache. As the
working set grows the ratio falls monotonically after $10^5$ --- $2.62$, then
$1.11$, then back to $1.88$ --- because (A) leaves the cache first and by a wide
margin. Between $10^5$ and $5\times10^5$ pairs (A)'s read slows by $2.9\times$
while (C)'s slows by $1.25\times$; past that both are memory-bound and the ratio
recovers toward, but does not reach, the instruction ratio.

\paragraph{The crossing point is predicted by \S\ref{sec:evalspace}'s space
constants, which is the check that this is a footprint effect.} A multi-edge pair
costs (A) $295.7$ bytes and (C) $69.2$, using the engine-level per-pair figures
of \S\ref{sec:evalspace}. Those constants put (A) past $100$\,MB at about
$3.4\times10^5$ pairs and (C) at about $1.5\times10^6$. The measured cliffs sit
between $10^5$ and $5\times10^5$ for (A), and between $5\times10^5$ and
$2\times10^6$ for (C) --- both bracketing the prediction, from an independent
measurement. So the $5.9\times$ space penalty \S\ref{sec:evalspace} reports for
(A)'s containers is not only a memory-footprint result. It is also a latency
result, and it is the reason (C)'s read closes most of a $4\times$ instruction
deficit at scale.

\paragraph{At $k=1$ it runs the other way.} Instructions put (C) at $1.15\times$
(A) flat. Time puts it at $1.30\times$ at $10^5$ pairs and $1.72\times$ at
$8\times10^6$ --- the deficit widens as residency drops. This one cannot be a
footprint effect: \S\ref{sec:evalspace} measures both designs at $23.1$ bytes per
all-inline pair, equal to three significant figures, and our own
\texttt{memory\_usage} reports $23.8$ across the sweep. We do not have a
decomposition of the residual, which is $85$\,ns at the largest size; the honest
statement is that it exists and that the instruction metric does not see it.

\paragraph{What this does to the paper's method.} \S\ref{sec:eval} chose
instruction counts for reproducibility and defended the choice on the grounds
that they measure work rather than machine. That defence stands --- the flat
control columns are it --- but the cost is now quantified rather than
hypothesised, and it is not a conservative bias in one direction. The metric
overstates (C)'s multi-edge deficit by about $2\times$ out of cache and
understates its single-edge deficit by about $1.5\times$. A reader should not
treat any instruction ratio in this paper as a proxy for time at graph sizes past
cache, in either direction. The two effects do not cancel; they act on the two
regimes the design is meant to trade between.
...
of $5.9$. So the bulk of the space difference between the designs is a fixed
per-pair charge, which is exactly the shape \S\ref{sec:cost} predicted from the
container object and is not what a reader would guess from
Table~\ref{tab:evalwrite}'s whole-graph megabytes. The boundary confirms the
container to the byte: (A) costs $+272.00$ bytes for the second identifier and
$+8.00$ for each one after it.

\paragraph{The per-identifier slope, and an ordering that inverted.} This paper
previously reported that each identifier after the second costs $8.04$ bytes in
(A) against $9.09$ in (C), and concluded that (A) is marginally cheaper per
identifier and that the whole difference between the designs is therefore the
per-pair charge. Narrowing $me$'s column dimension (\S\ref{sec:narrow}) changed
that. Re-running the same harness on both sides:

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
\texttt{relation\_matrices\_sz\_mb} & (A) C & (C) before & (C) after \\
\midrule
$k = 1$ (all inline)    & $11$ & $11$ & $11$ \\
$k = 2$                 & $141$ & $33$ & $29$ \\
$k = 4$                 & $148$ & $41$ & $34$ \\
$k = 8$                 & $164$ & $59$ & $44$ \\
reference ($10^6 \times 1$) & $22$ & $23$ & $23$ \\
\midrule
slope over $k \ge 2$, B/id & $8.04$ & $9.09$ & $\mathbf{5.24}$ \\
\bottomrule
\end{tabular}
\end{center}

The "before" column reproduces the published figures exactly, which is what
licenses attributing the move to the narrowing rather than to the machine or the
toolchain. And the move inverts the ordering: (C)'s marginal cost per identifier
was $13\%$ above (A)'s and is now $35\%$ below it. The sentence this paper used
to carry --- that (A) is marginally cheaper per identifier, so the designs differ
only in a fixed charge --- is no longer true in either half. There is now a
per-identifier difference, and it runs the same way as the per-pair one.

Whole-graph totals moved with it on the same fixture: $19$, $49$, $73$, $124$\,MB
at $k = 1, 2, 4, 8$ before, against $17$, $41$, $59$, $93$ after, and jemalloc
live bytes at $k = 8$ from $141{,}000{,}648$ to $106{,}797{,}680$, a $24\%$
reduction.

Table~\ref{tab:evalwrite}'s space columns are \emph{not} re-measured here: they
come from a different fixture and harness ($|E| = 176{,}000$, the benchmark
suite) which was not re-run. Its (C) live-bytes and \texttt{GRAPH.MEMORY} columns
therefore predate the narrowing, and the direction of the correction is the one
above --- (C)'s figures would fall, by an amount that grows with multiplicity and
is zero at $\mu = 1$. The ratios in that table should be read as upper bounds on
(C)'s space rather than as current values.
...
($2^{60}$ rows by $2^{31}$ columns, row keys) costs $34.63$ bytes per
...
steeply as its rows fill --- $20.32$ bytes at $k = 2$, $12.66$ at $4$, $8.83$
at $8$, $6.91$ at $16$ --- which is the hypersparse row overhead being
amortised, and the reason (B)'s penalty is worst exactly where (B) is supposed
to be uniform.

Every one of these auxiliary figures is exactly $4.00$\,B/id below the value
this paper reported before \S\ref{sec:narrow}'s narrowing, and the two adjacency
figures are unchanged --- which is the signature of the change being the column
index array and nothing else. Re-running the same harness at the commit before
the narrowing reproduces the old column to the hundredth ($38.63$, $24.32$,
$16.66$, $12.83$, $10.91$), so the difference is attributable to that one
declaration rather than to a toolchain move.
...
\[
  \begin{aligned}
    \text{(B)} \;&:\;
      ...
      \;+\; \underbrace{34.63}_{\text{one id per row in } me\text{'s geometry}}
      \;=\; 38.64 \ \text{B/edge},
    \\[2pt]
    \text{(C)} \;&:\;
  ...
  \end{aligned}
\]
a ratio of $3.22$. That ratio \emph{fell} from $3.55$ when $me$'s columns were
narrowed, and the direction is worth stating plainly: the narrowing helps (B)
more than it helps (C), because the auxiliary matrix is the whole of what (B) is
made of and is empty in (C) on a simple graph. An optimisation to this design
narrowed its own advantage over the alternative. (C) still wins the axis by
better than $3\times$, but a reader should not read $3.22$ as an improvement on
$3.55$. What the model assumes is that such an implementation would
...
\paragraph{The compound key restricted node identifiers --- and the restriction
is gone.} This paragraph is kept as a record of a limitation the artifact has
since removed, because both the defect and the shape of the repair are the
transferable part.

$\kappa$ was $(s \ll 32) \mathbin{|} d$, guarded by an assertion that both
endpoints fit $32$ bits. The guard was checking the wrong bound. The key is used
as a \emph{row index} of $me$, whose declared row dimension is
$\texttt{GrB\_INDEX\_MAX} = 2^{60}-1$, so a valid index needed
$\kappa(s,d) < 2^{60}$, i.e.\ $s < 2^{28}$ --- a bound $16$ times tighter than
the guard enforced. The guard therefore never fired on the case that mattered.

What made this more than a documentation gap is where the failure surfaced.
\texttt{Matrix::set} checked its GraphBLAS status under \texttt{debug\_assert!}
only, so in a release build the out-of-range write was \emph{dropped silently}:
the pair kept its $\M$ sentinel over an empty $me$ row. That is Invariant
\emph{promotion completeness} violated --- and its consequences are exactly the
ones \S\ref{sec:invariants} predicts for that invariant. Reads returned nothing
for a pair that had edges, and $\textit{edge\/count}$ disagreed with both the
inserts and the reads. Reproduced at two edges per pair:

\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
& $\kappa < 2^{60}$ & ids read back & $\textit{edge\/count}$ \\
\midrule
$s = 2^{27}$ & true  & $[10, 11]$ & $2$ \\
$s = 2^{28}$ & false & $[\,]$     & $1$ \\
$s = 2^{29}$ & false & $[\,]$     & $1$ \\
...
The repair is the block-indexed key of \S\ref{sec:compound}, and it is worth
recording where the shipped form differs from the one this paper first proposed,
because two of the differences matter.

\emph{The block is a pair, not a scalar.} The proposal split only the source,
$\kappa(s,d) = (s \gg k,\ ((s \bmod 2^{k}) \ll 32) \mathbin{|} d)$, which leaves
$d$ at $32$ bits and merely moves the ceiling onto the destination. Splitting
\emph{both} endpoints at $k = 30$ removes it from both: two $30$-bit halves fill
the $60$-bit budget exactly, and the block absorbs everything above.

\emph{The guard is gone rather than corrected.} The proposal kept a check,
derived from the storage instead of from $32$-bit packing, on the argument that a
loud failure beats silent corruption. That argument was right about the
predecessor and is moot for the successor: a key built from masked halves cannot
leave range, so there is no failure to be loud about. Totality is a better
answer than a correct assertion.

Two predictions held. The repair is \emph{migration-free}, because the
serialised form stores $s$ and $d$ separately and re-keys on load. And the
mechanised statement of injectivity did have to be restated for pairs --- and
that restatement was the improvement the paper claimed it would be: $\kappa$'s
injectivity, its range, and its inverse are now unconditional, and the
hypothesis that carried the old width assumption through the development is
deleted outright (\S\ref{sec:mechanised}).

One prediction was wrong in a way worth noting: the proposal argued for a sparse
map keyed by block, to keep per-transaction work proportional to populated
blocks. Populated blocks is the right quantity, but a map is the wrong
structure for it. Every real graph has exactly one block, so the artifact holds
blocks in a list sized for one with block $(0,0)$ at index $0$: a compare rather
than a hash, and nothing allocated. The general case still needs the fold over
live blocks, and that cost is linear in their number.
...
\emph{Fan-out is measured at the boundary only to $k = 16$}
(\S\ref{sec:evalfanout}), which is where this revision closed the gap the
previous one named here --- and closed it against us, by inverting the slope
claim. Above $16$ nothing is measured on either engine. Degree
(\S\ref{sec:evalentry}) is the exception in the other direction: it is priced
only at $k \in \{1, 2\}$, and its ratio is the one that grows with $k$, so
$4.21\times$ should be read as a floor rather than a figure.

\emph{Residency is measured on one machine only.} The previous revision listed
warm sequential reads as the limitation; access order (\S\ref{sec:evalfanout}),
residency (\S\ref{sec:evalcold}), transposed iteration, and the remaining entry
points (\S\ref{sec:evalentry}) are now all measured on both engines. What that
leaves is narrower and harder: the residency result is a wall-clock result, taken
on one macOS arm64 machine with one cache hierarchy, and its magnitudes are the
part most likely to move on different hardware. Its \emph{direction} at $k=2$ is
anchored by \S\ref{sec:evalspace}'s space constants, which are machine-independent
and predict the crossing point; its direction at $k=1$ is not anchored by
anything and rests on the measurement alone.

\emph{Only instruction counts are trustworthy here, with one deliberate
exception.} Some runs overlapped other work on the machine, so wall clock and
cycles are not reported, and no claim in \S\ref{sec:eval} should be read as a
latency result --- except \S\ref{sec:evalcold}, where a latency result is the
whole point and no instruction count could substitute for it. That subsection
carries its own method (minimum of three, ratios rather than magnitudes) and its
own weaker warrant; it is not evidence for any other number here. Everything is macOS on
...
A later re-run, on the same machine and across the commit that introduced the
block-indexed key and the narrow $me$ (\S\ref{sec:compound},
\S\ref{sec:narrow}), separates that library drift from this design change and
finds the design change is not where the read constants moved:
...
\midrule
inline read                 & $726.9$   & $721.9$   & $-0.7\%$ \\
sentinel read               & $2{,}922.8$ & $2{,}928.8$ & $+0.2\%$ \\
promote (add a 2nd edge)    & $3{,}174.2$ & $3{,}229.8$ & $+1.8\%$ \\
add a 3rd edge (control)    & $1{,}417.3$ & $1{,}478.7$ & $+4.3\%$ \\
\bottomrule
\end{tabular}
\end{center}

Reads are flat to within measurement noise, which is what the design predicts: a
read of an inline cell never consults $me$ at all, and a sentinel read now costs
one extra comparison to select a block from a list of length one. The write path
pays that comparison twice over --- promotion writes two identifiers --- and
$4.3\%$ on the third-edge control is the clearest signal in the table of what
blocking costs. Both are small enough that the ordering of any ratio in
\S\ref{sec:eval} is unaffected.

Two things follow for the reader. The absolute read constants in
Table~\ref{tab:evalboundary} are the $10.3.1$ figures and reproduce neither on
$10.5.0$ nor on \texttt{main}; the \emph{space} figures reproduce exactly, which
is the asymmetry \S\ref{sec:evalspace} exploits to attribute the narrowing. And
whole-graph resident numbers have a second mover this paper does not otherwise
cover: the same change replaced the edge-endpoint side table with a tiered
structure, which is outside the tensor but inside
Table~\ref{tab:evalwrite}'s megabytes.
...
\paragraph{The block-indexed key is landed; the many-block regime is not
measured.} The ceiling is removed and both acceptance measurements this section
previously asked for were taken (\S\ref{sec:limits}): a graph confined to block
$0$ pays nothing on reads and $1.8$--$4.3\%$ on the write path, and maintenance
cost is linear in live block count. What remains unmeasured is the regime that
motivated the question --- a graph whose multi-edge pairs are spread thinly
across \emph{many} blocks, where per-transaction fold and wait work becomes
proportional to the number of live blocks rather than to one. Reaching it needs
node identifiers above $2^{30}$ on both axes, which no fixture here builds, so
the crossover is known to exist and its location is not known.

\paragraph{The counter and the checked arithmetic are both done; the general
point is not retired.} The invariant set is proved (\S\ref{sec:mechanised}), but
proofs bind the model, not the running process: a memory error, a decoder fed a
corrupt blob, or a future edit to a path the model does not cover can still break
an invariant in production. Mechanisation and run-time checking answer different
questions, and the first does not retire the second. That is why the two
paragraphs below are kept after the work in them shipped --- the reasoning
transfers to the next invariant someone is tempted to leave unchecked.

Two of the three repairs in \S\ref{sec:limits} were prototyped, which answered
the question this paragraph previously left open --- in the negative, the useful
outcome. Two have since shipped: checked arithmetic, immediately below, and the
block-indexed key of \S\ref{sec:compound}.

\emph{Checked arithmetic is free, and has since been done.} Computing
...
was an allocation abort --- arbitrarily far from the cause, and
indistinguishable from genuine memory pressure --- rather than a wrong answer.

The artifact now does this, and the shape of the change is the part worth
keeping. Each of the five steps is checked, and a failure panics naming all six
operands, so the term whose invariant broke is in the message. The cost is
nothing: six metadata reads and five arithmetic operations either way. And the
regression test has to \emph{forge} its input --- a tombstone with no committed
entry beneath it, which violates $dm \subseteq m$ --- through a test-only hook,
because no mutation path can reach that state. That is the honest statement of
what the check is for: not a reachable bug, but the difference between a loud
stop and a wrong number reaching an allocator when something outside the model's
reach breaks an invariant. It is the same trade the compound key used to make
for key aliasing, and \S\ref{sec:limits} explains why that particular guard is
now gone instead.
...
\paragraph{The modelling gaps are closed; the byte stream is not.} Three places
where the code was ahead of the proof, all three since mechanised, and the one
that remains is smaller than any of them.

Iteration was mechanised by its \emph{result} --- the effective set restricted to
a row range --- rather than as the three-way lookahead merge that computes it. It
is now the merge (\texttt{Tensor/Merge.lean}), which is where the delicate index
reasoning lives. Batch deletion was the most urgent, being the one place the code
moved \emph{after} the proof was written: to make demotion linear in the batch
(\S\ref{sec:evalosc}) the implementation reads each pair's $me$ row once, replays
that pair's transitions into a plan, and applies the plan in a separate write
phase, and ``batching by pair and replaying is equivalent to the sequential
per-edge fold'' was an obligation the model did not state. It states it now
(\texttt{Tensor/RemovePlan.lean}): the per-pair plan agrees with
\texttt{foldl removeOne} restricted to that pair, and plans for distinct pairs
commute, the disjointness that makes the two-phase split sound. And the
\texttt{Encode}/\texttt{Decode} direction that was worth having --- not
round-tripping but the guarantee that a malformed blob is \emph{rejected} rather
than decoded into a tensor violating the invariants --- is proved in
\texttt{Tensor/CodecCheck.lean}.

What is still outside the model is the serialised \emph{byte stream}: framing,
lengths, endianness. The model reasons over the decoded tables, so it says what a
well-formed blob means and nothing about which byte sequences are well-formed.
That is the remaining gap, and it is a parsing obligation rather than a
data-structure one.

\paragraph{Stop constructing an iterator per call.} The largest single constant
this paper measures is not a representation choice. Per-call
\texttt{GxB\_Iterator} construction is five sixths of a degree call
(\S\ref{sec:evalentry}) and sits under every point read in
Table~\ref{tab:evalboundary}; the same scan through a re-seeked iterator costs
$298$ instructions against $1{,}951$ through a fresh one. Design (A) pays it once
per degree, this design pays it once per row scan and again per multi-edge pair.

A handle cache is the obvious repair and is the wrong one --- a previous revision
of this paper reported one, it was never committed, and its figures are withdrawn
in \S\ref{sec:evalentry}. What the cost actually argues for is a \emph{reusable
cursor} threaded through the hot callers, so a traversal attaches once and
re-seeks per row. That is an API change rather than a local optimisation, which is
why \#2572 stopped where it did: it took what was reachable without a new cursor
API and no more. Sizing it needs one number this paper does not have --- how many
rows a typical traversal reads through one logical cursor --- because the saving
is per attach avoided.

\paragraph{Give the fold policy a read-side term.} \S\ref{sec:evaldecomp}'s
two-state read was a resident delta on $me$ making every multi-edge read consult
a layer it could skip --- a flat ${\approx}1{,}200$ instructions per read,
independent of how much the delta held, so one pending identifier cost what eight
thousand did. The targeted repair was one probe: $me$'s delta is hypersparse and
keyed by $(\kappa, e)$, so ``does the delta touch this row'' is far cheaper than
building a merged iterator, and a delta is small by construction so nearly every
row takes the base path. \textsc{EffGet} already did exactly this for $dm$
(\S\ref{sec:reads}); the multi-edge path had simply never learned the trick. It
has now, and the two-state read is gone.

The cause was narrower than the symptom suggested. \texttt{Iter::from\_layers}
asked whether a delta was empty \emph{in total}, not whether it held anything in
the row being read, so one pending identifier anywhere made every single-row scan
allocate, attach and seek an iterator that yielded nothing. The repair answers the
narrower question without touching GraphBLAS: a bitmap of which rows a delta may
hold, allocated only once the delta holds something and dropped when it folds,
whose only required property is that it never says \emph{no} when the answer is
yes --- so the one choke point every entry-adding path goes through invalidates it
by default, and a bulk path added later without knowing about it costs a scan
rather than a wrong answer. A skipped layer is \emph{detached} rather than
dropped, because \texttt{seek} re-points an existing iterator and a layer dropped
on the first range would silently swallow entries in every later one.

Measured on the reported repro, instructions per pair as the graph grows:
the step that made this look like two discrete states was $+1{,}143$ and is now
$+116$.

The more interesting repair is the fold policy itself. The square-root rule of
\S\ref{sec:folding} is derived entirely from what a delta costs \emph{writers}
--- the rewrite tax $wD/(2t)$ against the fold bill $F/D$ --- and has no term at
all for what a resident delta costs \emph{readers}. This defect is a direct
consequence: a one-entry delta is nearly free to fold and, measured, expensive to
keep, and no threshold derived from write amortisation will ever fold it. What
the model is missing is a term in the read rate.

\paragraph{Demotion policy --- measured, and the upside is small.} Demotion is
eager: the moment a pair drops to one edge, its identifier returns inline and its
$me$ row is emptied, so a workload oscillating across the boundary pays a
promotion \emph{and} a demotion per cycle. Hysteresis --- keeping a pair in $me$
until the transaction ends --- would trade space for stability, and which wins was
not obvious. The question was downstream of \#2429 (\S\ref{sec:evalosc}), since
while a batch's demotions cost more than linearly in their number any measurement
of hysteresis would mostly have measured the defect. With that fixed the
comparison was run, and it answers itself: a full oscillation costs $9{,}486$
instructions per pair, of which $7{,}106$ are the same $+1/-1$ work on a
three-edge pair that never crosses the boundary. Hysteresis can therefore remove
at most $2{,}380$ instructions --- a quarter of the cycle --- and costs
$46.2$\,bytes per oscillating pair held to commit. Three quarters of what looks
like transition cost is not the transition. The policy is left eager, and this
paragraph is the reason rather than an intention.
...
That statement now exists as a companion note, \emph{Inline-First with Sentinel
Promotion: A Pattern for Sparse Stores Whose Cells Usually Hold One Value}
(\texttt{pattern.tex} in this artifact). It says as little about edges as it
can, and it carries three things this paper cannot: the invariant set restated
as obligations an adopter inherits rather than properties this design has, an
explicit account of when \emph{not} to use the pattern --- including a candidate
instantiation we sketch and then argue against --- and the measurement traps of
\S\ref{sec:evalfanout} and \S\ref{sec:evalentry} generalised, since both of the
attributions they correct were ours and in print.
...
Landed changes corresponding to points in the text: the fold policy of
\S\ref{sec:folding} is PR \#2349; the demotion fix of \S\ref{sec:evalosc} is
\#2431 closing issue \#2429; the removal of the \texttt{multi\_count} field
(\S\ref{sec:limits}) is \#2439; the cardinality identity's checked arithmetic is
\#2565; the block-indexed key and the narrow $me$ of \S\ref{sec:compound} are
\#2579 closing \#2578; the degree entry point of \S\ref{sec:evalentry} is \#2572.
The other anomaly this evaluation found, the two-state point-read cost, was
issue \#2430 --- diagnosed and closed by \#2571, described below.
