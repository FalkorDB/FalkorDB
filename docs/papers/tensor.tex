...
\texttt{graph/src/graph/graphblas/tensor.rs} on the \texttt{main-rs} branch of
the FalkorDB repository at commit \texttt{a30d4ca}. Our evaluation
(\S\ref{sec:cost}) is analytic: we derive space and operation costs from the
storage formats and the GraphBLAS operations invoked, and we do not report
end-to-end query benchmarks. We flag this plainly because the design's
central claim --- that inline-first is cheaper for the common case --- is a
claim about constants, and constants deserve measurement; \S\ref{sec:future}
says what we would measure.
...
\]
and when a delta exceeds a threshold (10{,}000 entries) it is folded into the
base and cleared --- structurally the same amortisation as an LSM tree's
compaction \cite{oneil1996lsm}, at one level. Each physical matrix sits behind
...
order-independent, so they can be applied under independent size thresholds
--- the artifact folds each at $10{,}000$ entries.
...
\textsc{Flush}            & $2$ bulk ops, amortised & threshold $10{,}000$; order-free by Inv.~\ref{inv:pure} \\
...
\paragraph{Make \texttt{multi\_count} checkable.} A debug-mode assertion of
Theorem~\ref{thm:count} against a scan would convert the whole invariant set
into a testable property, at a cost paid only in debug builds.
...
\texttt{versioned\_matrix.rs}, \texttt{matrix.rs} and \texttt{cow.rs}, on the
\texttt{main-rs} branch of \url{https://github.com/FalkorDB/FalkorDB} at
commit \texttt{a30d4ca}. It is built against SuiteSparse:GraphBLAS v10.3.1
and LAGraph v1.2.1. The C implementation used for comparison as design (A) is
