...
two-state read is a resident delta on $me$ making every multi-edge read consult a
layer it could skip. The targeted repair is one probe: $me$'s delta is
hypersparse and keyed by $(\kappa, e)$, so ``does the delta touch this row'' is
far cheaper than building a merged iterator, and a delta is small by construction
so nearly every row would take the base path. \textsc{EffGet} already does
exactly this for $dm$ (\S\ref{sec:reads}); the multi-edge path simply never
learned the trick.
