...
\paragraph{What this does not establish.} Two limits are worth stating plainly,
because a mechanisation invites the assumption that everything is settled.

First, \emph{no cost claim is mechanised, and none could be usefully.} The
development says what each operation computes, never how long it takes. Every
performance statement in this paper --- the operation table of
\S\ref{sec:cost}, the fold constants $F$ and $w$, the claim that the phase
split of \S\ref{sec:insert} keeps insertion linear --- rests on the analytic
model and on measurement, and would survive or fall entirely independently of
the proofs. The one place the two meet is negative and useful:
\texttt{edgesAt\_flush\_decision\_irrelevant} shows the fold policy cannot
affect what the tensor denotes, which is what licenses tuning it against
measurements without re-examining correctness.

Second, the proofs bind a \emph{model}, and the model's correspondence to the
