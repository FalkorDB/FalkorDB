...
order. We further analyse two classes of correctness hazard that arise from
embedding a data structure in a non-blocking sparse-linear-algebra runtime:
\emph{value typecasting}, where using a value-carrying operator instead of a
pattern-only operator silently destroys the edge whose identifier is $0$; and
\emph{deferred computation}, where a resize leaves the ``committed'' base with
queued work and violates an invariant every read path depends on. Both were
observed in practice and are now pinned by regression tests. We close with an
analytic cost model, a comparison against the pointer-tagged
container-per-cell design used by FalkorDB's C implementation, and a
discussion of the domain restrictions the compound key imposes.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Two hazard classes from the host runtime}
        (\S\ref{sec:hazards}). \emph{Value typecasting}: GraphBLAS
        \texttt{eWiseAdd} propagates a single-sided operand's \emph{value},
        so building a Boolean deletion mask from a \texttt{UINT64} matrix
        turns the edge whose identifier is $0$ into a \texttt{false} entry,
        which valued masks then treat as absent --- a silently lost deletion.
        \emph{Deferred computation}: \texttt{GrB\_Matrix\_resize} may queue
        pending work on the committed base, violating an invariant asserted by
        every read path, and only in debug builds. Both are analysed and
        pinned by tests.
  \item \textbf{An analytic cost and space model}
        (\S\ref{sec:cost}) and an explicit account of the design's
        \emph{domain restrictions} (\S\ref{sec:limits}), including the
...
commits given under Availability. Our evaluation (\S\ref{sec:cost}) is
analytic: we derive space and operation costs from the storage formats and the
GraphBLAS operations invoked, and we do not report end-to-end query benchmarks.
We flag this plainly because the design's central claim --- that inline-first
is cheaper for the common case --- is a claim about constants, and constants
deserve measurement; \S\ref{sec:future} says what we would measure. The one
place where we do rely on measured constants is the fold policy of
\S\ref{sec:folding}, whose two cost parameters are measured rather than derived
precisely because the rule they determine is sensitive to them.
...
requires; the rest of this paper is largely about that price.
...
pub struct Tensor {
    ...
    dp: Cow<Matrix<u64>>,        // pending additions / in-place value updates
    dm: Cow<Matrix<bool>>,       // pending deletions (mask over m)
...
Two asymmetries are deliberate. First, the forward adjacency is \emph{not}
wrapped in the generic \texttt{VersionedMatrix}; the tensor owns its three
layers directly, because the delta semantics it needs (\S\ref{sec:deltas})
are strictly richer than the Boolean ones and the edge-identifier meaning of
the values must stay local to this module. Second, the backward matrix
...
status, which a transaction cannot change (only a fold can), so no transition
crosses bands. Within a band, adding and removing identifiers moves left and
right, and every cycle returns to the canonical state --- $\mathbf{D}
...
Note what folding does to the automaton: it collapses the shadowed states.
$\mathbf{F} \to \mathbf{E}$, $\mathbf{I} \to \mathbf{D}$, $\mathbf{H} \to
\mathbf{D}$, and $\mathbf{G},\mathbf{J} \to \mathbf{A}$. The ten states are a
transaction-local phenomenon; a quiescent tensor occupies only
...
% =====================================================================
\section{Two hazards from the host runtime}
\label{sec:hazards}

Both hazards below were found empirically, both are now regression-tested in
the artifact, and both generalise beyond this data structure: they are what
happens when a data structure with its own invariants is layered on a runtime
with typed operators and deferred execution.

\subsection{Value typecasting destroys the edge whose identifier is $0$}
\label{sec:hazard-zero}

Consider building the Boolean deletion mask of
Equation~\eqref{eq:dmmask} with the obvious operation: a masked
\texttt{eWiseAdd} that copies the selected cells of the \texttt{UINT64} base
$m$ into the \texttt{BOOL} matrix $dm$. \texttt{eWiseAdd} propagates cells
present on only one side, \emph{typecasting} the value; the \texttt{UINT64}
value $0$ becomes the \texttt{BOOL} value \texttt{false}. GraphBLAS
distinguishes a stored \texttt{false} from an absent entry structurally, but a
\emph{valued} mask --- the default --- treats a stored \texttt{false} as
absent. Therefore:

\begin{quote}
Deleting the edge whose identifier is $0$ writes a stored \texttt{false} into
$dm$; every later valued-mask use of $dm$ skips it; the deletion is silently
lost, and the edge remains readable.
\end{quote}

The bug is invisible in almost every test, because identifier $0$ is one
value out of $2^{60}$ --- and unavoidable in production, because identifiers
are allocated densely from $0$, so the \emph{first edge ever created} is the
one affected. The fix is to use operations that provably never read operand
values: \texttt{eWiseMult} over \texttt{GxB\_ANY\_PAIR\_BOOL} for the mask
intersection, and \texttt{GrB\_Matrix\_apply} with \texttt{GxB\_ONE\_BOOL} for
pattern unions (the artifact's \texttt{set\_pattern}). The artifact's
regression test builds $10{,}000$ edges with identifiers $0 \ldots 9999$,
folds them into the base so that identifier $0$ genuinely lives in the
\texttt{UINT64} base, deletes identifier $0$ through the fast path with a
non-zero control, and asserts on both the point read and the materialised
pattern.

The general lesson is a typing discipline: \emph{in a system where matrix
values are identifiers rather than numbers, an operation that can read a value
is an operation that can corrupt state.} Pattern-only operators should be the
default, and any valued operator on an identifier-carrying matrix should be
justified at the call site.

\subsection{Deferred computation breaks the ``base is never pending'' invariant}
\label{sec:hazard-resize}

Every read path calls \textsc{WaitFwd}, which materialises the deltas and
\emph{asserts} that the committed base is not pending. The assertion is
justified by an argument about who writes the base: only a fold does, and a
fold materialises. \texttt{GrB\_Matrix\_resize} is the counterexample. Growth
that is large enough to change the internal representation leaves pending
work on the base, after which every read path's assertion fires.

Two properties made this hazard nasty. It is \emph{debug-only}: the assertion
compiles out in release, where the consequence is instead a base read
without materialisation. And it is \emph{scale-dependent}: growing capacity
from $10{,}001$ to $10{,}501$ queues nothing and the bug hides, whereas the
growth to $114{,}688$ that a real workload produced --- creating and deleting
$100{,}000$ temporary nodes against a graph loaded from a snapshot, which is
what makes relationship tensors exist to be resized at all --- crashed the
server through
$\textsc{Commit} \to \textsc{DeleteImplicitEdges} \to \textsc{Iter}$.

The fix is to materialise immediately after resizing, at the one site that
writes the base outside a fold. This costs nothing on hot paths --- resize
happens only on capacity growth --- and the same hole existed, and was closed
the same way, in the generic \texttt{VersionedMatrix}. The lesson generalises:
\emph{when a runtime defers work, ``I did not write this object'' is not the
same claim as ``this object has no queued work,''} and any invariant of the
second form needs enumerating every call that can queue work, including the
ones that look like metadata changes.
...
is out of range. The gap is unreachable at present scales and only for
multi-edge pairs --- a simple graph never indexes $me$ --- but it is a
discrepancy between a checked precondition and an actual one, and the natural
repair is to derive the guard from the storage: assert
$\kappa(s,d) < \mathrm{nrows}(me)$, or widen $me$'s declared dimension. Since
$me$ is hypersparse, widening costs nothing except that GraphBLAS's own index
limit is $2^{60}$, so the real fix is a narrower packing (e.g.\ $30 + 30$
bits) or a non-packed key.
...
\paragraph{\texttt{multi\_count} is incrementally maintained.}
It cannot be recomputed in $O(1)$ (\S\ref{sec:invariants}), so
Theorem~\ref{thm:count} depends on every mutation path updating it. A
divergence would surface as a wrong --- possibly wrapped --- edge count rather
than as corrupt data, but it would surface late.
...
As stated in \S\ref{sec:intro}, our costs are analytic. The claims most in
need of measurement are the ones about constants: the single-edge read path
against design (B)'s two-lookup path, allocator behaviour under churn against
design (A), and the promotion/demotion cost on workloads whose multiplicity
oscillates around $2$ --- the one regime where (C) does strictly more work
...
\paragraph{Measurement.} The experiment we would run first is a multiplicity
sweep: fix $|E|$, vary $\mu \in \{1, 1.1, 2, 4, 16\}$, and measure resident
size, single-pair lookup latency, full-scan throughput and bulk-insert
throughput for (A), (B) and (C). The interesting region is $\mu$ slightly
above $1$, where a small fraction of pairs forces promotions and (C) pays a
transition that neither alternative has.

\paragraph{Tighten the key.} Derive the compound-key guard from
$\mathrm{nrows}(me)$ rather than from $32$-bit packing
(\S\ref{sec:limits}), which removes the discrepancy and makes the bound
self-documenting.

\paragraph{Make \texttt{multi\_count} checkable at run time.} The invariant set
is now proved (\S\ref{sec:mechanised}), but proofs bind the model, not the
running process: a memory error, a decoder fed a corrupt blob, or a future edit
to a path the model does not cover can still break an invariant in production.
A debug-mode assertion of Theorem~\ref{thm:count} against an independent scan
would catch that, at a cost paid only in debug builds. Mechanisation and
run-time checking answer different questions and the first does not retire the
second.
...
non-blocking runtime with typed operators adds two hazard classes that are
easy to state after the fact and were not obvious before: a value-propagating
operator silently destroys the edge whose identifier is $0$, and a resize
leaves the ``committed'' base with queued work in violation of an assertion
every read path makes. Both are the same underlying mistake --- assuming an
operation does less than it does --- and both are the price of building a
data structure with its own invariants inside someone else's execution model.
