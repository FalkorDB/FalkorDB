...
Which leaves the third repair as the one worth landing. Rather than police the
counter, remove the obligation: seal $me$ and its counter into one type whose only
mutators move both together, as \S\ref{sec:components}'s delta layers already do
with their own counts and fold latches. Every mutation of $me$'s rows is already
confined to the tensor's own module, and the single external reader is read-only,
so the encapsulation is available today. It converts ``every mutation path must
remember'' into ``two functions do it'' --- checkable by reading a type instead of
auditing call sites --- and it subsumes the run-time check rather than
complementing it. Checked arithmetic still earns its place afterwards, since
encapsulation cannot defend against a corrupt decoded blob.
