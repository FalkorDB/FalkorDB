...
column degree, $k=1$           & $1{,}432$ & $2{,}573$ & $1.80$ \\
column degree, $k=2$           & $1{,}581$ & $5{,}475$ & $3.46$ \\
\addlinespace
...
fixture and method as Table~\ref{tab:evalfanout}. The bulk paths are within
$17\%$; degree is not, and the reason is an entry point (C) does not have.}
\label{tab:evalentry}
\end{table}
...
(C) has no degree entry point at all. Degree is computed one level up, in
\texttt{Graph::get\_node\_outdegree}, by counting a full single-row iteration ---
which materialises every identifier in the row only to discard it. That reads
like the whole explanation, and the ratio's growth with $k$ ($1.96\times$ at
$k=1$, $4.21\times$ at $k=2$) reads like confirmation of it.
...
\paragraph{Both have since been addressed, which changes two numbers in this
paper.} Recycling \texttt{GxB\_Iterator} handles through a small thread-local
free list --- sound because GraphBLAS documents re-attaching an iterator to
another matrix --- removes about half the per-call construction cost, and a
\texttt{row\_degree}/\texttt{col\_degree} that drops the per-pair buffer and
shares one $me$ cursor across a row removes the rest of the degree-specific
overhead. Measured on the same fixtures: degree falls to $1{,}203$ and $2{,}793$
at $k = 1, 2$ against (A)'s $1{,}011$ and $1{,}160$, so
$1.97\times \to 1.19\times$ and $4.15\times \to 2.41\times$; the transposed
figures fall to $1{,}814$ and $3{,}409$, $1.80\times \to 1.27\times$ and
$3.42\times \to 2.16\times$. The multi-edge point read of
Table~\ref{tab:evalboundary} falls from $2{,}940$ to $2{,}200$, taking its ratio
against (A) from $3.89\times$ to $2.91\times$.

Table~\ref{tab:evalentry} reports the engine as measured for this paper, before
those changes, and the numbers above are what the same benches say after. We keep
both rather than silently restating the better ones: the point of the
decomposition is that a plausible explanation survived a whole revision because
nothing had asked it to account for the total.
