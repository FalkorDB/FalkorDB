...
$1.8\times$ and $5.1\times$, which is the sharpest argument we can offer for
...
\begin{enumerate}
  ...
  \item \textbf{An analytic cost and space model} (\S\ref{sec:cost})
        ...
        attributions overstate storage cost by $1.8\times$ and $5.1\times$. The
...
\S\ref{sec:eval} measures it --- where it turns out to be, even after a defect
fix, about three times what (A) pays for the same logical change, and where the
row of this table it most decisively confirms is instead the \emph{absence} of
(A)'s heap container: $0.24\times$ its promotion cost and $0.25\times$ its space
per multi-edge pair.
...
storage turn out to be overstated by $1.8\times$ and $5.1\times$.
...
puts it lower again, at $1{,}707$, which is the figure to prefer and which makes
the engine-level charge $17\%$ high; \S\ref{sec:evalspace} adds the one thing
...
(\S\ref{sec:evaldecomp}). At the boundary it is $+1{,}707$ for (C) and $+198$
for (A). The engine-level figures overstate the storage-attributable penalty by
$1.8\times$ for (C) and $5.1\times$ for (A). The equal-$k$ decomposition did
better than raw differencing for (C) --- its $1{,}993$ is only $17\%$ above the
boundary --- and no better at all for (A), whose $1{,}013$ is the same
$5.1\times$ high. So the caveat was not pro forma: an engine-level difference
attributed to a data structure was wrong by a factor of five on one of the two
sides, and in the direction that flatters neither design consistently.
...
\emph{entry} charge for the multi-edge path --- $+1{,}707$ for (C) against
...
identifier from $k = 2$ to $k = 8$: $448$ for (C) against $854$ for (A). (C)
pays a large entry and then walks cheaply; (A) pays a small entry and then walks
expensively. The crossover is what the decomposition already reported, and the
boundary locates its left endpoint far lower than engine-level measurement could
see.
...
arm64, with the C side pinned to SuiteSparse:GraphBLAS v10.3.1; Linux and CI are
unmeasured.
...
corrects it on the transition, the cost the design nominated as its own: that
cost was a quadratic in our implementation until we instrumented and fixed it,
and even now it is about three times what a container costs to allocate. The
general lesson is less about tensors than about grain. Two of the attributions
we made from whole-query measurements were wrong by factors of $1.8$ and $5.1$
...
(\texttt{counters.py} in that tree); both engines were release builds on one
macOS arm64 host, with the C side built against SuiteSparse:GraphBLAS v10.3.1.
The boundary harnesses of \S\ref{sec:evalboundary} and \S\ref{sec:evalspace}
are committed at \texttt{bench/studies/edge\_storage/} and
\texttt{graph/src/graph/graphblas/tensor\_cost\_bench.rs} on the
\texttt{measure/design-b-and-boundary} branch; the C harness there links the
shipped static archive directly, with no stubbed symbols.
Of the anomalies this evaluation found, the batch demotion cost is FalkorDB issue
\#2429, fixed by PR \#2431, and the two-state point-read cost is \#2430, open
