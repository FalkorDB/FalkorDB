...
Lean 4 against a model transcribed from the Rust --- $5{,}061$ lines and $362$
...
\begin{enumerate}
  ...
  \item \textbf{A machine-checked development, and an account of its limits}
        (\S\ref{sec:mechanised}). $5{,}061$ lines and $362$ theorems of Lean 4
...
\begin{itemize}
\item \textbf{\texttt{Tensor}} ($4{,}093$ lines, $290$ theorems).
...
about the abstract encode/decode pair); and batched deletion replays a per-pair
plan where the model folds per-edge transitions (\S\ref{sec:future}). One such
divergence has since been closed rather than documented: the model carried
