...
Lean 4 against a model transcribed from the Rust --- $5{,}070$ lines and $362$
...
\begin{enumerate}
  ...
  \item \textbf{A machine-checked development, and an account of its limits}
        (\S\ref{sec:mechanised}). $5{,}070$ lines and $362$ theorems of Lean 4
...
the discipline is gone. We keep it in the invariant set because the mechanised
model still carries the field, and because the cardinality identity below is
stated against it.
...
\begin{itemize}
\item \textbf{\texttt{Tensor}} ($4{,}102$ lines, $290$ theorems).
...
about the abstract encode/decode pair); batched deletion replays a per-pair
plan where the model folds per-edge transitions (\S\ref{sec:future}); and the
model still carries $\textit{multi}$ as a field with
Invariant~\ref{inv:counter} as its agreement condition, where the code now
derives it from $me$ (\S\ref{sec:limits}). The last of these is a divergence
that can only make the code safer than the model --- a derived quantity cannot
disagree with itself --- but it is a divergence, and the theorems below are
about the field.
...
second. The harness now slices by the header's own column ranges. The failure is
not a small bias but a catastrophic one --- a fused field moves the parsed total
by $\approx\!7\times10^{16}$ bytes, so an affected reading is off by orders of
magnitude rather than by percent --- and no figure in
Table~\ref{tab:evalwrite} is of that character. We report the numbers as
measured and note that the shape of the failure is what licenses trusting them,
rather than a re-run under the corrected parse.
...
convert a wrapped count into an error raised where it is detected. Second, the
mechanised model still carries \texttt{multiCount} as a field with
Invariant~\ref{inv:counter} as its agreement condition. Every operation in the
model provably preserves it, so the \emph{algorithm} was never in question; what
was unproved is that the Rust performed the same updates the model does. Deleting
the field discharges that particular correspondence obligation in the code while
leaving the model one revision behind it (\S\ref{sec:mechanised}).
...
algorithm in this paper; nothing here has been re-measured against it, so the
numbers should be read as of the commit under Availability rather than as of
\texttt{main}.
...
their work having landed on \texttt{main}, so a reader holding that PDF should
look here rather than for those branches.
...
\texttt{bench/studies/\allowbreak edge\_storage/} and
\texttt{graph/\allowbreak src/\allowbreak graph/\allowbreak graphblas/\allowbreak tensor\_cost\_bench.rs},
the C side of which
links the shipped static archive directly with no stubbed symbols --- are
\emph{not} on any published branch at the time of writing. Until they are, the
boundary results of \S\ref{sec:evalboundary} should be read as reported rather
than as reproducible, which is a weaker standing than the rest of the evaluation
has and one we would rather state than paper over.
