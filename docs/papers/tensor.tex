...
\paragraph{Degree is the one place (C) pays for a missing entry point rather
than for its representation.} (A) implements degree as a scan of the row's
...
which materialises every identifier in the row only to discard it. Its cost is
therefore proportional to the number of \emph{edges}, and the ratio grows with
$k$: $1.96\times$ at $k=1$, $4.21\times$ at $k=2$, and by construction worse
above that.

This is a fixable gap rather than a property of inline-first storage. The
information (A) reads in constant time --- the number of identifiers held for a
pair --- is equally available in (C) as the cardinality of the pair's row in
$me$, and a degree that reads the forward cell and, on the sentinel, takes that
cardinality without materialising the row would have (A)'s shape. We have not
implemented it, so the table reports what the engine does today.

Its blast radius is bounded and worth stating so the number is not read as worse
than it is: these four functions back Cypher's \texttt{indegree} and
\texttt{outdegree} built-ins and nothing else. Traversal does not route through
them. A workload that never calls those built-ins never pays this.
