...
order. We further state two rules that embedding such a
...
analytic cost model and a measured multiplicity sweep against the
pointer-tagged container-per-cell design used by FalkorDB's C implementation,
which confirms the design's read, write and allocator claims --- and refutes our
own prediction that its distinguishing promotion cost is a small constant: it is
a small constant in the C implementation and is, in ours, currently
super-linear in the number of pairs one transaction promotes. We report that,
its issue number, and the domain restrictions the compound key imposes.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{An analytic cost and space model} (\S\ref{sec:cost})
        \textbf{and a measured multiplicity sweep against the
        container-per-cell implementation} (\S\ref{sec:eval}). Instruction
        counts at fixed $|E|$ over $\mu \in \{1, 1.1, 2, 4, 16\}$, with the
        read path additionally decomposed --- by varying only the number of
        edges per pair --- into an inline read, a fixed charge for the
        multi-edge path, and a per-identifier slope. Scans, inserts and allocator behaviour come out as the design
        predicts; the promotion/demotion transition does not, and we report the
        failure and its issue number rather than the prediction.
...
\texttt{graph/src/graph/graphblas/tensor.rs} in the FalkorDB repository, at the
commits given under Availability. Our cost model (\S\ref{sec:cost}) is
...
claim about constants and constants deserve measurement. That evaluation is a
multiplicity sweep against design (A) as shipped in FalkorDB's C
implementation, plus a controlled decomposition of the read
path. It confirms the read and write claims, and it refutes the paper's own
prediction about the promotion/demotion transition: the design says a small
constant, our implementation currently pays a cost that grows with the size of
the batch (\S\ref{sec:evalosc}). Two of the three designs remain
unimplemented, and \S\ref{sec:limits} is precise about what that leaves
unmeasured. The fold policy of \S\ref{sec:folding} rested on measured constants
...
\S\ref{sec:eval} measures it --- where it turns out to be, in our
implementation, far larger than this table's ``two auxiliary writes'' suggests.
...
pub struct Tensor {
    ...
    multi_count: u64,            // number of pairs whose effective value is MULTI
...
Write $|X|$ for $\mathrm{nvals}$, $\textit{multi}$ for \texttt{multi\_count},
and $P^{\ast} = \{p : \varepsilon(p) \neq \bot\}$ for the effective pattern.
...
Invariant~\ref{inv:counter} deserves a note: $\textit{multi}$ is maintained
incrementally and is not derivable from the matrices in $O(1)$ (deriving it
would require counting sentinel-valued cells, i.e.\ a scan). It is therefore
the one piece of state whose consistency rests on every mutation path
updating it, rather than on a structural argument --- a genuine, if small,
soundness liability, and one reason the cardinality identity is worth stating
as a theorem to be tested against.
...
The slow path is per-edge and is the only place demotion happens. For a
...
$D \ge \text{base}/K$. A ratio is right when each entry is rewritten $O(1)$
...
$F$ and $w$ are measured rather than modelled. The fold is $\approx 82\%$
fixed cost and independent of $\mathrm{nrows}$ --- an empty fold costs
$2.35\,\mu\text{s}$ at $\mathrm{nrows} = 65\text{k}$, $1\text{M}$ and
$16.7\text{M}$ alike --- so $F$ is flat in the base above about $10^6$
entries, and the balance point does not track base size. And $w$ differs by
...
development is in \texttt{proofs/} in the artifact and comes in two layers, the
second discharging the first's one abstraction boundary:

\begin{itemize}
\item \textbf{\texttt{Tensor}} ($\approx$3{,}950 lines, $\approx$290 theorems).
...
\item \textbf{\texttt{VersionedMatrix}} ($\approx$940 lines, $\approx$73
...
Per-pair automaton& \ref{sec:automaton}& nine \texttt{trans\_*} lemmas, one per documented arrow \\
\bottomrule
\end{tabular}
...
Where the code is currently ahead of the model, we say so: iteration is
mechanised by its result rather than as the three-way lookahead merge, and the
serialised byte format is outside both layers (the round-trip theorems are
about the abstract encode/decode pair).
...
count. First, a Boolean layer whose entries are all \texttt{true} is
\emph{iso-valued}, and SuiteSparse then stores one shared scalar instead of a
value array --- so such a layer costs its index array alone; a non-iso Boolean
layer costs one further byte per entry. Second, $me$'s declared dimensions of
...
\textsc{EdgeCount}        & $O(1)$ metadata $+$ one \texttt{eWiseMult} & intersection skipped when $|dp| = 0$ (Thm.~\ref{thm:count}) \\
...
single-pair lookup cost, bulk-insert cost and resident size. We then descend
from whole queries to the read path itself, because the sweep alone turned out
not to support the reading we first gave it.

Two of the design's predictions survive. Inline-first is cheaper on the common
case, by margins that are largest exactly where the design says they should be
and vanish exactly where it says they should vanish; and the absence of
per-pair heap containers shows up as an allocator curve that does not have
(A)'s hump. The third prediction fails, and it fails against us: (C)'s
distinguishing cost is the promotion/demotion transition, which the design
says is a small constant --- and which \emph{is} a small constant in (A), and
is in our implementation a cost that grows with the number of pairs a single
query moves across the boundary. That is \S\ref{sec:evalosc}, and it is
reported there rather than around it.
...
are tens of percent on queries that run in milliseconds. Wall clock was
recorded and is consistent with every ordering reported below, but it is not
what we report.
...
\paragraph{Granularity, which is the real methodological problem.} These are
\emph{engine-level} measurements, and the tensor is one component of each
engine. The queries are chosen so that edge storage dominates --- a full scan
of one relationship type, bound-pair lookups in a loop --- but a ratio here is
a ratio between two engines, not between two data structures.
\S\ref{sec:evaldecomp} is where this stops being a caveat and becomes a
finding: one row of the sweep invites an explanation in terms of the tensor,
and a controlled measurement at the read path shows that explanation is wrong.
...
$|E| = 176{,}000$ throughout, so scan cost falls with $\mu$ on both sides
because $|P|$ falls. Ratios below $1$ favour (C). The two lookup rows where
(C) loses are \emph{not} explained by the tensor --- see
\S\ref{sec:evaldecomp}.}
\label{tab:evalread}
\end{table}
...
dereferencing a tagged pointer. And (C)'s cost per additional identifier is
about half (A)'s, $448$ against $854$, because scanning a hypersparse row is
cheaper per identifier than walking a container.

Those numbers arrange themselves neatly. (C)'s inline advantage, $993$
instructions, almost exactly funds its excess fixed charge, $980$; the two
cancel at $k = 2$ ($4{,}576$ against $4{,}589$, parity to $0.3\%$), and from
$k = 3$ upward the slope advantage compounds and (C) wins outright --- $0.92$
at $k = 3$, $0.75$ at $k = 8$. We also checked that the fixed charge is not
...
$1{,}993$ at equal $k$ is the figure to use. Both remain upper
bounds on (B) rather than measurements of it, for the reason
\S\ref{sec:limits} gives: a sentinel read also scans a row, and (B) is not
implemented.
...
\toprule
 & \multicolumn{3}{c}{Bulk insert, instructions} & & \multicolumn{2}{c}{live bytes} & & \multicolumn{2}{c}{\texttt{GRAPH.MEMORY}} \\
\cmidrule(lr){2-4}\cmidrule(lr){6-7}\cmidrule(lr){9-10}
$\mu$ & (A) C & (C) Rust & (C)/(A) & & (A) & (C) & & (A) & (C) \\
\midrule
$1.0$  & $6{,}957{,}009{,}801$ & $1{,}643{,}493{,}402$ & $0.24$ & & $18{,}904{,}728$ & $17{,}106{,}496$ & & $8$\,MB  & $10$\,MB \\
$1.1$  & $6{,}530{,}671{,}571$ & $1{,}605{,}780{,}449$ & $0.25$ & & $22{,}440{,}928$ & $17{,}206{,}176$ & & $12$\,MB & $10$\,MB \\
$2.0$  & $4{,}690{,}043{,}030$ & $1{,}549{,}215{,}127$ & $0.33$ & & $42{,}521{,}760$ & $19{,}855{,}248$ & & $29$\,MB & $11$\,MB \\
$4.0$  & $2{,}895{,}288{,}180$ & $1{,}273{,}515{,}766$ & $0.44$ & & $28{,}908{,}296$ & $19{,}910{,}864$ & & $17$\,MB & $9$\,MB \\
$16.0$ & $1{,}934{,}337{,}954$ & $\phantom{0}993{,}911{,}383$ & $0.51$ & & $17{,}801{,}784$ & $20{,}786{,}536$ & & $8$\,MB  & $8$\,MB \\
\bottomrule
\end{tabular}
...
the build; \texttt{GRAPH.MEMORY} reports whole megabytes, which is coarse at
this scale. Note (A)'s non-monotonic space curve, peaking at $\mu = 2$.}
\label{tab:evalwrite}
\end{table}
...
Resident size is comparable overall, better for (C) in the middle of the range,
and slightly worse at the top. The line worth a sentence of its own is (A)'s,
...
hold sixteen identifiers. (C) has no such term: promotion moves identifiers
into a matrix that was already allocated, and its live bytes rise gently and
monotonically across the whole sweep, $17.1$\,MB to $20.8$\,MB. This is the
allocator-pressure claim of \S\ref{sec:cost} showing up as the shape of a
curve, which is stronger evidence for it than any single point would be. At
$\mu = 16$ (C) is the worse of the two on live bytes ($20.8$ against $17.8$),
and we do not have an account of that beyond the wasted sentinel-holding
forward cell per promoted pair that Table~\ref{tab:space} predicts.
...
\subsection{The oscillation regime, where the design's own prediction fails}
\label{sec:evalosc}
...
measurement that matters most, and it is the one that comes out badly.
...
\bottomrule
\end{tabular}
\caption{The transition, isolated (left) and as a function of how many pairs
one query demotes (right; instructions per pair for the demoting delete, the
same quantity as the first column). Both engines pay essentially the same price
...
Table~\ref{tab:evalosc} gives it. Emptying a one-edge pair costs the two
engines almost the same ($8{,}390$ against $8{,}810$), which is the control
working: the delete machinery is not what separates them. The transition costs
(A) $2{,}322$ instructions per pair --- a small constant, as the design says it
should be --- and costs (C) $219{,}955$, roughly $95\times$ more.

Worse, it is not even a constant. Growing the number of pairs a single query
...
demotion measured while $|me|$ grows $17\times$ moves only from $162{,}919$ to
$166{,}353$ per pair. So the blow-up is driven by the number of demotions in
the batch, not by how much has overflowed --- which is what one would expect if
the batch's demotions are being processed in a way that is quadratic in their
count rather than streamed.

We claim this as an implementation defect and not a property of the design, and
the claim is falsifiable in the obvious way: the design demotes a pair with two
auxiliary writes, (A) implements the same logical transition for a constant
$2{,}322$ instructions, and (C)'s own cost is independent of $|me|$ and
therefore not paying for the structure the design puts it in. It is tracked as
FalkorDB issue \#2429
(\url{https://github.com/FalkorDB/FalkorDB/issues/2429}). Until it is fixed,
the honest statement of the design's position is the one this section measures
rather than the one \S\ref{sec:designspace} predicts: inline-first buys the
common case at the price of a transition, that price is a small constant in a
mature implementation of the alternative, and in ours it is currently
super-linear in the size of the batch that pays it. A workload that drives many
pairs across the one-to-two boundary in single transactions is, today, the
workload to keep away from this engine.
...
fits. Rebalancing to $30+30$ buys one bit over four ($\approx 10^9$ per side)
and postpones rather than removes the ceiling.
...
\paragraph{\texttt{multi\_count} is incrementally maintained --- but need not be.}
Theorem~\ref{thm:count} depends on every mutation path updating the counter, and
a divergence would surface as a wrong --- possibly wrapped --- edge count rather
than as corrupt data, so it would surface late and far from its cause. In the
artifact the exposure is concrete: seven call sites add or remove a row entry of
$me$, and three of them adjust the counter.

The premise that makes this look unavoidable is that the quantity cannot be
...
proportional to $|E|$. The counter is therefore a \emph{cache} of a structural
property of $me$, not independent state, which admits three repairs of
increasing ambition.

\emph{Make divergence loud rather than silent.} The subtractions in
Theorem~\ref{thm:count} can only underflow if an invariant is already broken ---
that is exactly what the mechanised \texttt{edgeCount\_no\_underflow} says --- so
computing the identity in checked arithmetic converts a wrapped count into an
error raised where it is detected. This costs nothing and removes the worst
failure mode without addressing the cause.

\emph{Check it, cheaply enough to leave on.} Comparing the counter against $me$'s
vector count is $O(\textit{multi})$, so unlike the full Theorem~\ref{thm:count}
audit against a scan --- which is $O(|E|)$ --- it can run at every commit rather
than only in debug builds. That converts a late, remote symptom into an
immediate, local one.

\emph{Or remove the obligation.} Either compute the count on demand from $me$ at
each \textsc{EdgeCount}, paying one metadata read on a path that is not hot; or
keep the field and make forgetting it impossible by construction, bundling $me$
with its counter in a type whose only mutators move both together. The second is
the move the delta layers already make (\S\ref{sec:components}), and it is the
one that generalises: it turns ``every mutation path must remember'' into ``two
functions do it,'' which is a property a reviewer can check by reading a type
rather than by auditing call sites.

Note what the mechanisation does and does not settle here. Every operation in the
model provably preserves Invariant~\ref{inv:counter}, so the \emph{algorithm} is
not in question; what is unproved is that the Rust performs the same updates the
model does. That is the correspondence assumption of \S\ref{sec:mechanised}, and
the third repair is valuable precisely because it shrinks the surface over which
that assumption has to hold.
...
\S\ref{sec:eval} measures designs (A) and (C) against each other; four gaps
remain, and they are not small.

\emph{Design (B) is emulated, not implemented.} What
\S\ref{sec:evaldecomp} isolates is the fixed charge for taking the multi-edge
path, which is the constant (B) would pay on every read; that bounds (B)'s read
cost from above and says nothing at all about its space, and nothing about its
\emph{absence} of promotion cost --- which, given \S\ref{sec:evalosc}, is the
whole of (B)'s case. A design whose distinguishing weakness is a second lookup
we have now priced at $1{,}993$ instructions deserves a better answer than an
upper bound derived from its competitor.

\emph{Nothing is measured at the data-structure boundary on (A)'s side.}
Every number in \S\ref{sec:eval} comes from a query against a whole engine, and
the tensor is one component of each. Differences are therefore attributable to
two engines, of which edge storage is one part, and the decomposition of
\S\ref{sec:evaldecomp} narrows that only for (C), by varying $k$ on otherwise
identical graphs rather than by instrumenting the structure itself. Closing this
properly means a C harness around \texttt{src/graph/tensor/} on \texttt{master},
measuring the same operations as calls rather than as queries. Issue \#2430 is
what happens when one does not have that: an engine-level number pointed at the
wrong component, and it took a controlled experiment to see it.

\emph{The oscillation figure measures a defect, not the design.}
\S\ref{sec:evalosc} reports what our implementation currently costs on the
one-to-two boundary. It does not report what inline-first costs there, and the
two differ by roughly two orders of magnitude. Until \#2429 is fixed, that row
of the comparison is unavailable.
...
measurable on a synthetic sweep, settles it. One machine, one platform and one
relationship type is also the whole of the evidence.
...
\paragraph{Take the counter off the critical path.} The invariant set is now
proved (\S\ref{sec:mechanised}), but proofs bind the model, not the running
process: a memory error, a decoder fed a corrupt blob, or a future edit to a path
...
The repairs in \S\ref{sec:limits} are what we would do first, and they are
ordered by how much they change. Checked arithmetic in
Theorem~\ref{thm:count} and a commit-time comparison against $me$'s vector count
are both cheap enough to leave enabled in release, which is the interesting
claim: the obvious audit --- asserting the identity against a full scan --- is
$O(|E|)$ and therefore debug-only, whereas exploiting
Invariant~\ref{inv:promotion} makes the check proportional to the number of
multi-edge pairs. What we would want to measure before enabling it by default is
the cost of that vector-count read while $me$ carries live deltas, since that is
the one case where it is not a single metadata access.

\paragraph{Close the two remaining modelling gaps.} Iteration is mechanised by
its \emph{result} --- the effective set restricted to a row range --- rather
than as the three-way lookahead merge that computes it, and the
\texttt{Encode}/\texttt{Decode} blob format is outside the model on both
layers. Those are the two places where the code is ahead of the proof, and the
merge is the one with delicate index reasoning, so it is the more valuable of
the two to close.
...
obvious which wins. That question is downstream of \#2429
(\S\ref{sec:evalosc}), though: while a batch's demotions cost more than linearly
in their number, the policy choice is not what dominates, and a measurement of
hysteresis would mostly measure the defect.
...
The implementation described here is \texttt{Tensor} in
\texttt{graph/src/graph/graphblas/tensor.rs}, with supporting code in
\texttt{versioned\_matrix.rs}, \texttt{matrix.rs} and \texttt{cow.rs}, in
\url{https://github.com/FalkorDB/FalkorDB}. This revision describes the
\texttt{fold-policy} branch at commit \texttt{6458a2a}, which adds the fold
policy of \S\ref{sec:folding}; the Lean development of
\S\ref{sec:mechanised} is under \texttt{proofs/} on the \texttt{tensor-proof}
branch at commit \texttt{0a10c93} (that branch is \texttt{fold-policy} plus the
proofs). The measurements behind $F$ and $w$ are reproducible from
\texttt{fold\_cost\_bench.rs} in the same tree.

Both branch from \texttt{main}, the Rust engine's trunk, at commit
\texttt{02049e0}. Note that the earlier revision of this paper cited a
\texttt{main-rs} branch; the Rust engine's trunk is now \texttt{main}, and
\texttt{master} carries the C implementation used for comparison as design (A),
under \texttt{src/graph/tensor/}.

The measurements of \S\ref{sec:eval} were taken with the repository's benchmark
harness under \texttt{bench/}, whose instruction counting is
\texttt{proc\_pid\_rusage} against the server process
(\texttt{counters.py} in that tree); both engines were release builds
on one macOS arm64 host. The sweep driver itself is not yet committed on either
branch above, so \S\ref{sec:evalmethod} states the construction of each
configuration in enough detail to rebuild it. The two anomalies the evaluation
reports are FalkorDB issues \#2429 (batch demotion cost) and \#2430 (the
two-state point-read cost).

Built against SuiteSparse:GraphBLAS v10.3.1 and LAGraph v1.2.1; the proofs
against Lean 4 v4.32.0 with mathlib v4.32.0, pinned in the project's
