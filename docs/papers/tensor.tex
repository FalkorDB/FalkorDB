...
Table~\ref{tab:evalboundary} reports. (A)'s half is a standalone C harness linking the
shipped static archive with no stubbed symbols. It is \emph{not} on any published
branch at the time of writing, so (A)'s boundary columns should be read as
reported rather than as reproducible --- a weaker standing than the rest of the
evaluation has, and one we would rather state than pass over. It does build and
run from a \texttt{master} worktree plus that archive, which is how
\S\ref{sec:evalfanout}'s (A) columns were obtained; what is missing is
publication, not the artifact.
