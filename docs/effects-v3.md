...
exception — a record is self-describing before its rows. `AttrValues` is the one
part that follows the `IdList`, because it is per row rather than per record.

| record | **partition key** — the `GROUP BY` | per row |
| --- | --- | --- |
| `CREATE_NODE` | `LabelSet` + `AttrIds` | id, values |
| `CREATE_EDGE` | `RelType` + `AttrIds` | id, src, dst, values |
| `UPDATE_NODE` | `LabelSet` + `AttrIds` | id, values |
| `UPDATE_EDGE` | `RelType` + `AttrIds` | id, values |
| `SET_LABELS` / `REMOVE_LABELS` | `LabelSet` | id |
| `DELETE_NODE` | `LabelSet` — the labels the node *actually* carried | id |
...
**A compressed payload states its frame's length, so it is self-delimiting.** A
reader bounds the decompress input from the header rather than from wherever the
buffer happens to stop, which is what lets a payload be carried inside a larger
stream and what lets a reader without its own bounds checks size its input
safely. Bytes after the declared frame are a decode error, not padding: they mean
the header and the payload disagree about where the frame ends, and nothing after
that can be trusted.

The checksum is taken over the **plaintext**, not the frame — those are the bytes
the records are parsed from, and it stays reproducible across zstd versions, which
may frame the same input differently. CRC-32/ISO-HDLC, the zlib polynomial, so C
matches it without vendoring anything.
...
| `AttrIds` | `u16 n` · `u16 attr_id × n` — the shape's attribute ids, stated **once** |
| `AttrValues` | `SIValue × (count × n)` — row-major, row *k* for the k-th id |

There is no fused `AttrSet` block on the wire. The ids travel with the shape,
before the `IdList`; the values follow it. Nothing writes the two adjacently.
...
Segment := u8 header · payload
  header bits 0-1  kind: 0 = Range, 1 = Ascending, 2 = Repeat; 3 unused
         bits 2-3  value width code   (Range base, Repeat id)
         bits 4-5  count width code   (Range len,  Repeat count)
         bit  6    descending
         bit  7    reserved, MUST be zero — a decoder rejects a header that
                   sets it rather than masking it off

  Range     := uint(value_width) base · uint(count_width) len
  Repeat    := uint(value_width) id   · uint(count_width) count
  Ascending := u32 blob_len · roaring64[blob_len]     portable format
```

**Direction is bit 6, not a kind of its own.** A descending segment is the same
payload read the other way: a descending `Range`'s `base` is its *first and
highest* id rather than its first and lowest, and `Ascending`'s blob is a
**set**, which has no direction at all — the ascending and descending forms of
the same ids are byte-identical apart from that one header bit. `Repeat` has no
direction (one id, however many times), so the bit on a repeat is a malformed
segment and a decoder must reject it rather than ignore it.

**When a descending segment is emitted — normative, because two encoders that
disagree here produce different bytes for the same write.** Direction is decided
by the *second* id of a run, and the rules mirror the ascending ones exactly:

1. A lone id has no direction. It is written as `Range { base = id, len = 1 }`
   with bit 6 **clear**, whichever way the ids around it go.
2. The id after it decides. `id + 1 == base` makes the segment descending and
   extends it to `len = 2`; `id == base + len` extends it ascending. Two ids
   consecutive and descending are therefore **always** one descending `Range`,
   never two segments.
3. A descending run extends while each id steps down by exactly one, and ends at
   the first id that does not — including one that steps *up*, because a
   reversal ends a run rather than continuing it in the other direction.
4. A descending run of ranges collapses to a `Descending` bitmap under the same
   cost comparison as the ascending case, over the same id *set*. Sortedness is
   never used to make a collapse eligible: a descending run collapses because it
   is a run, not because its ids could be re-ordered.

So `9,8,7,6,5` is one `Range { base: 9, len: 5, descending }` and not five
ascending ranges of one, and an encoder that produces the latter is not merely
less compact — it produces a different, non-canonical buffer for the same write.
`9,7,5,3,1` is five ascending ranges of one, because five gapped ids are below
roaring's floor and nothing collapses; the same shape at four thousand ids is
one `Descending`.

Rules 1-4 bind the **encoder**. A decoder still has to say what it does with
the non-conforming form, and the answer is: **accept a `len = 1` `Range` with
bit 6 set.** Both readings name the same single id, so the segment is
non-canonical rather than unintelligible, and a decoder that keeps the observed
bit round-trips it unchanged instead of silently rewriting a peer's buffer.

That is deliberately the opposite of the `Repeat` ruling, and the line between
them is worth stating because both look like "a directionless thing with a
direction bit". For a `Repeat`, direction is not part of the encoding at all —
there is no ascending or descending repeat — so a peer setting the bit is using
a bit this format does not define for that kind, and it means something the
build does not understand. For a `Range`, the bit *is* part of the encoding and
its meaning at `len = 1` is well defined. **Refuse what cannot be interpreted;
accept what is merely non-canonical.** Canonicality is an encoder obligation:
byte-for-byte agreement between engines rests on both encoders producing the
canonical form, which rule 1 gives, and is not weakened by a decoder tolerating
the other one.

**A range must fit the id space it describes, and a decoder must check both
ends.** The two numbers in a `Range` are independent on the wire, so nothing in
the widths or the reserved bit stops them from crossing a boundary: descending,
`base` counts down and `base - (len - 1)` can pass below zero; ascending,
`base + (len - 1)` can pass above `u64::MAX`. Either way the ids named do not
exist, and on unsigned arithmetic the descending case wraps to near `u64::MAX`
and turns seven wire bytes into a request to expand four billion ids — inside
apply, on the main thread. So:

```text
  descending   len - 1 <= base
  ascending    len - 1 <= UINT64_MAX - base
```

and a segment failing either is **malformed**, not clamped. Do not simplify
this to `base >= len`. It does not merely over-reject — it *under*-rejects,
which is the dangerous direction:

| | legal? | `base >= len` says |
| --- | --- | --- |
| ascending `base=0 len=5` | legal | **no** — refuses the common dense-from-zero case |
| descending `base=7 len=8` | legal | **no** — refuses the exact-zero boundary |
| ascending `base=U-3 len=8` | malformed | **yes** — accepts an overflow past the top |
| ascending `base=U-7 len=8` | legal | yes (agrees) |
| descending `base=3 len=8` | malformed | no (agrees) | Note what these are
not: they are derived from the segment's own two numbers, so they can only
reject a segment that is internally inconsistent and cannot fire on anything an
encoder would produce. That is what separates them from a ceiling — a cap on how
many ids a record may carry, or how deep a value may nest, is a guess about
legitimate data and will eventually refuse some.

The `roaring64` blob is the **portable** serialization — a `u64` bucket count,
then per bucket a `u32` high word followed by a standard roaring32 stream — so
`roaring64_bitmap_portable_deserialize_safe` reads it directly. Written after
`optimize()`, which is normative rather than a tuning knob: an unoptimized
bitmap serializes to different bytes, so two engines that disagree about calling
it produce different buffers for the same set.
...
## Cardinality: when a block may be empty

The spec said nothing about this, and the cost was measurable. A review found
that of every block whose cardinality can be zero, exactly one had both a
present and an empty case in the 33-case conformance corpus. That is why a
decoder which refused `CREATE (:Person)` — the commonest write in the language —
could hold a fully green test suite on both engines.

**A record is vacuous if removing an empty block leaves it saying nothing about
any entity it names. Vacuous records must not be emitted.**

That single rule decides every case, and the three categories fall out of it.

**An empty block that describes the entities is information, and is legal.**
`DELETE_NODE` with no labels says *these nodes carried no labels*. `CREATE_NODE`
with no attribute ids says *these nodes have no properties*. The record still
states something about the entities it names, so the empty block is a fact
rather than an absence.

| legal and degenerate | the query that produces it |
|---|---|
| `CREATE_NODE.attr_ids` | `CREATE (:Person)` |
| `CREATE_EDGE.attr_ids` | `CREATE (a)-[:R]->(b)` |
| `CREATE_NODE.labels` | `CREATE ({x: 1})` |
| `UPDATE_NODE.labels` | a `SET` on an unlabeled node |
| `DELETE_NODE.labels` | deleting an unlabeled node |
| `CREATE_INDEX` options | an index created with no configuration — every presence byte 0 |

**An empty block that is the record's whole payload makes it vacuous, and must
not be emitted.** A label record's entire content is its label set; with no
labels it says "add nothing to these nodes", which is an instruction to do
nothing rather than a statement about them. Nothing survives removing the block.

| must not be emitted | why |
|---|---|
| `SET_LABELS.labels` | the label set is the payload |
| `REMOVE_LABELS.labels` | same |
| `UPDATE_*.attr_ids` | an update that updates nothing |
| `CREATE_CONSTRAINT.props` | a constraint over no properties |
| `DROP_CONSTRAINT.props` | the properties identify which constraint |
| `count == 0`, any record | names no entities, so it cannot describe any |

`count == 0` is refused **at the header**, before any block is parsed, because
`count` sizes every block that follows: one check there keeps a zero-length
parse path out of each of them, rather than requiring two engines to agree on
all of those paths separately.

The others are suppressed by the emitter and **tolerated as a no-op by the
reader**, which is deliberately not what `count == 0` does. Refusing a
zero-label record removes no parse surface, because the zero-length `LabelSet`
path is required anyway by `DELETE_NODE` — so refusal buys nothing and costs a
forced-resync loop against any peer still emitting one. Refuse to emit;
tolerate on read.

**A schema announcement is exempt.** `ADD_SCHEMA` and `ADD_ATTRIBUTE` are
*bindings*, not instructions: they state that a name exists at an id, which the
receiver needs in order to keep its dictionaries aligned whether or not any
record in the same buffer references it. Their content **is** the information,
so they are legal even when nothing uses them — a payload that registers a label
and then leaves it unused is well-formed.

### Removing every attribute

`SET n = {}` has no special representation. **Removal is stated as `T_NULL` per
attribute, and there is no sentinel attribute id.** v2 carried one —
`ATTRIBUTE_ID_ALL`, `USHRT_MAX - 1` — as a per-record field. v3 has no place to
put it and no need for it.

The reason is ordering, not tidiness. v2's sentinel was a per-record field; in v3
an attribute id lands in the record's **shape**, shared by every entity in it. So
`SET n = {} SET n.x = 1` would produce the shape `{x, ALL}` — and v3 guarantees
no ordering, within a record or between them, so nothing could say which
applied first. A dedicated "remove all" record has the same problem one level
up: `REMOVE_ALL` plus an `UPDATE` is two records whose relative order would have
to matter. Both options need ordering semantics the format deliberately lacks.

Stating the attributes explicitly needs none, because the emitter resolves to the
**end state** before emitting rather than replaying operations:
`SET n = {} SET n.x = 1` is one staged map, `{a: null, b: null, x: 1}`, and one
record. There is nothing to order because there is only one statement.

A sentinel would also be undetectable. A shape naming something that is not an
attribute is not a structural error, so no bounds check on either side could
see it — the payload would decode cleanly and apply wrongly.

**Grouping is still available.** Stating attributes explicitly means the shape is
each entity's own attribute set, so a `SET n = {}` over a heterogeneous
population fragments into one record per distinct set. That is a grouping choice,
not a property of the format: because every value in such a record is already a
removal, the shape may be widened to the union of the sets with every value
`T_NULL`, and the extra slots remove attributes those entities never had.

This is the one case where padding a shape is safe. In general it is not — a pad
and a removal are the same byte, so grouping different-but-overlapping shapes
would silently delete a property the primary still holds (see
`emit::gather_rows`). When every value is a removal, the pad and the intent
coincide. Whether an engine takes that optimisation is an implementation choice
with identical observable effect, like any other write-side grouping.

### Where the guarantee ends

**The format guarantees the bytes for a given record. It does not guarantee
which records a given query produces.**

Two engines may emit different-but-equivalent records for the same query and
both be correct. A write-side optimisation is an implementation choice:
`MATCH (n) SET n:L` on a graph where some nodes already carry `:L` may be sent
as every matched node or as only the newly-labeled ones. Both describe the same
resulting graph.

What makes that safe is the apply side. **Label add and remove are idempotent
set operations, and that is a contract on the reader**, not a description of
what emitters happen to send. It is also what lets an emitter send a whole
matched set without a per-entity read — so a reader whose label add is not
idempotent has not merely mishandled a redundant record, it has withdrawn the
licence the bulk emission path depends on.

Two consequences for testing:

- **No test may assert cross-engine byte equality for a payload derived from a
  query.** Record-derived fixtures are unaffected, which is why the conformance
  corpus is built from `Record` values rather than from queries.
- The corpus therefore pins the wire format and says nothing about which records
  a query produces. That is the boundary of the guarantee, not a gap in the
  corpus: a query-level corpus would assert something the format does not
  promise.

**Where the corpus is.** On its own branch — `effects-v3-corpus`, 37 cases as
`.hex`/`.json` pairs, artifacts and nothing else, no engine code and no history
shared with `main`. It is deliberately not carried in an engine branch: both
implementations need one address for it that outlives whichever pull request
happens to be open. Beside a checkout:

```sh
git worktree add tests/fixtures/effects_v3 origin/effects-v3-corpus
```

The minimal test on the far side needs no JSON parser — read a `.hex`, decode
...
errors on its own. Note what it does *not* catch: a change applied
symmetrically to both directions. Only bytes someone else produced catch that,
which is the whole reason the corpus is checked in rather than generated on
each run.
...
Strings are `uint64 len` — the value's **byte length** plus one — then `len`
bytes, the last of which is a NUL. A payload may contain **interior NUL bytes**, so
a reader must honour the length and must never call `strlen` on the payload. The
terminator is written because C reads these into a `char[]` and treats them as C
strings; it is a convenience for the common case, not the authority on where the
value ends.

That is not a hypothetical. openCypher's `\uXXXX` escape produces such a string, so
`RETURN size('a\u0000b')` is 3 over the bytes `61 00 62`, and the value survives
storage and a `DEBUG RELOAD` intact. A writer taking the length from `strlen` emits
2 and `61 00`; this engine emits 4 and `61 00 62 00`. Two implementations reading
the same sentence, disagreeing on one value — which is why the sentence changed.
The corpus pins it as `value_string_interior_nul`.

For any string with no interior NUL the two rules agree byte for byte, which is why
this correction changed no existing corpus case and why the heading above still
says *unchanged*. They part company only on values v2 had no way to represent — so
a v2 reader carried over unmodified is right about every string v2 ever sent, and
wrong about this one.

`SIValue` is `uint32 SIType` then payload; `T_POINT` is 2×f32, `T_ARRAY` and
`T_VECTOR_F32` use `uint32` counts.

### `IndexOptions`, and why it is not a map

    u8 · string language                  0 = the statement said nothing
    u8 · u64 n_stopwords · string × n
    u8 · f64 weight
    u8 · u8 nostem                        1 or 0; anything else is refused
    u8 · string phonetic                  an algorithm code, e.g. "dm:en"
    if IndexFieldType & INDEX_FLD_VECTOR:
        u64 dimension                     no flag: a vector field must have one
        u8 · u64 M
        u8 · u64 efConstruction
        u8 · u64 efRuntime
        u8 · u64 simFunc                  VecSimMetric: 0 L2, 1 IP, 2 cosine

**There is no `T_MAP` on this wire, and `Value::Map` has no encoding at all.**
A payload claiming one is refused rather than skipped: it came from something
this format does not describe.

`OPTIONS {...}` used to travel as a map of stringly-typed keys, which was not a
decision — C's `create_index_effect.c` had an `SIValue` in hand at the call site
and passed the parser's representation straight through, and this side mirrored
it. Every other layer already holds these as typed fields: C's `IndexField` has
two nested option structs, C's v19 RDB writes them at fixed positions, and this
engine parses them into `IndexOptions` the moment they arrive. The option set is
closed — ten names, enumerated in both engines' structs — so nothing was gained
by making the wire generic, and a map cost a decoder agreement about key
framing, key order and unknown keys that nobody needed to reach.

The field order and vocabulary are `_RdbLoadIndexField`'s. RDBs have been
mutually loadable since #2459, so this is a shape both engines already
implement and cross-check, and the far side can reuse its own field logic.

**Every option carries a presence byte, and this is the one place the layout
departs from the RDB's.** The RDB writes defaults for what a statement omitted
and can afford to: it saves a whole index at once, so "weight 1.0" and "no
weight given" load the same index either way. An effect is an instruction
against an index that may already exist, and `create_index` refuses an explicit
language when one is already set for the label — so materialising defaults on
the wire diverges a replica with

    Can not override index configuration: Language is already set for label 'L'

which is how this was found, in `test_CRUD_replication`. Absence is
information. It is also cheaper: the all-absent record is 69 bytes against 113
for the same statement with defaults written out.

A byte per option rather than one mask for all of them. A mask is smaller but
has to be kept in step with the field list by hand — set the bit, write the
value, in two places, in the same order, forever.

`phonetic` is the algorithm code as a string, not a bool. C stores
`char *phonetic` and accepts `dm:fr`/`dm:pt`/`dm:es`; this engine narrows to a
bool and rejects them. A bool here would have baked one engine's narrowing into
the format. `simFunc` goes the other way and *is* the `VecSimMetric`
discriminant, because both engines' RDB encoders already publish that numbering
to stay RDB-compatible — it is shared format vocabulary like `IndexFieldType`
and `SIType`, not an internal representation. An unrecognised value is refused,
not read as L2.

**The vector half is gated on `IndexFieldType`, not on whether the writer has
vector options.** That is a correctness requirement rather than a style: the
reader only has the field type, which is already on the wire ahead of this, and
gating the two directions on different things desynchronises the stream — the
writer emits `u64`s the reader never consumes and the next record is parsed
from the middle of them. The text half is written whatever the field type, five
zero bytes saying "nothing said", so there is one gate rather than two.
...
## Changed in v3
    ...
    4 CREATE_EDGE
      ...
      v3   u32 count · RelType · AttrIds · IdList · IdList(src) · IdList(dst) · AttrValues
    ...
    5·6 DELETE_NODE / DELETE_EDGE
      ...
      v3   u32 count · LabelSet · IdList     | u32 count · RelType · IdList · IdList(src) · IdList(dst)

`DELETE_NODE` carries a `LabelSet` and it is not decoration: the labels are the
ones the node *actually* held, captured as it was deleted, and a replica needs
them to clear the right label-scoped index documents. The query's pattern cannot
supply them — `MATCH (n:A) DELETE n` over an `(:A:B)` node must clear `:B`'s
indexes too. Trust them rather than re-deriving: by the time the record applies,
the node is gone and its label matrices with it.

    7·8 SET_LABELS / REMOVE_LABELS
      v2   GrB_Index blob_size · GxB_Vector_serialize(nodes)[blob_size]
      v3   u32 count · LabelSet · IdList(nodes)
    ...
    11 CREATE_INDEX   — one record per STATEMENT, which v2 was not
      v2   one record per field
      v3   t · SchemaType · LabelID id · string label · IndexFieldType
             · u16 n · (AttributeID id, string name) * n · IndexOptions
    12 DROP_INDEX      — mirrors 11 without the options; a drop sends n = 0
    13 CREATE_CONSTRAINT
      t · ConstraintType · GraphEntityType · ConstraintStatus · LabelID id
        · string label · uint8 n · (AttributeID id, string name) * n
    14 DROP_CONSTRAINT — mirrors 13 **without** the status

Two things here that a reader built from v2 gets wrong.

**`CREATE_INDEX` is one record per statement, and `IndexFieldType` precedes the
field list.** v2 sent one record per field; two single-field records are *not*
equivalent to one two-field statement, because applying the second is refused
with "Can not override index configuration" — index-level options belong to the
index and cannot be set twice. So the field type is stated once, at the
statement level, ahead of a counted list of fields. A drop carries an empty
list and no options.

**`CREATE_CONSTRAINT` carries a `ConstraintStatus` that C does not send**, and
it is the one place v3 deliberately carries more than C. C's
`EffectsBuffer_AddCreateConstraintEffect` sends no status, so a C replica cannot
tell an enforcing constraint from one still being built. A replica never
validates, so the announcement is the only thing that can tell it — and it is
what makes the second announcement, the one after validation finishes, converge
on the first rather than duplicate it. `DROP_CONSTRAINT` has no such need and
omits the field; the apply path never reads it.
...
`OPTIONS {...}` now survives the trip. v2 dropped it on the wire and forced
those statements to replicate as verbatim `GRAPH.QUERY`; v3 carries it as the
typed block above, so the replica installs what the statement said — and only
what it said, since an option nobody gave arrives as absent rather than as a
default. `map_to_index_options` runs once, on the master at parse time, so a
malformed option is a decode error on a payload instead of an apply error on a
graph, and no string matching survives on the replica's side.
...
`build_effects` is set from replica presence *or* the AOF flag
(`src/graph_core.rs:766`):
...
so an instance with neither builds nothing at all. Under v2 the choice between
verbatim replay and an effects payload was made *after* that point, on a buffer
already constructed, so a query that then replicated verbatim had still paid to
build one. That is what made the verbatim path unusable as a baseline, and it is
why the figures below are measured against a replica-less server rather than
against a verbatim-replicating one.

`EFFECTS_THRESHOLD` is not part of this any more, and an earlier draft of this
section said it was. It survives only as `EFFECTS_THRESHOLD_DEPRECATED`
(`src/commands/config_cmd.rs`), stored and echoed by `GRAPH.CONFIG` and read for
no decision. That draft also named a `should_use_effects` as its consumer; no
such function exists in the tree. Both are recorded here rather than quietly
deleted, because a reader who implemented against that sentence would go looking
for a threshold that decides something.
