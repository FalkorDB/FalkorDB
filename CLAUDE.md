...
The `GB_jit__*.c` files under `build/graphblas/PreJIT/` are AUTOGENERATED by
`gen_prejit.sh` from GraphBLAS's runtime JIT. NEVER hand-edit them.

Kernels are vendored PER-PLATFORM (`PreJIT/linux/` and `PreJIT/macos/`),
because the JIT defn strings embedded in each kernel are captured after host
header macro expansion (e.g. Apple fortify headers rewrite `memcpy` to
`__builtin___memcpy_chk`). A kernel harvested on one OS can fail its `_query`
hash check on the other, and GraphBLAS then silently falls back to slow
generic kernels. `graphblas.sh` picks the right subdirectory via `uname`.

To regenerate, run `gen_prejit.sh` ON EACH platform:
- `linux/`: inside the Linux Docker toolchain image
  (`ghcr.io/falkordb/falkordb-build`) — the production/CI target.
- `macos/`: natively on macOS.
