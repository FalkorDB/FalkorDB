...
Linux Docker toolchain image (the production/CI target). Kernels harvested
on macOS can embed platform-specific JIT defn strings (e.g. Apple fortify
headers rewrite `memcpy` to `__builtin___memcpy_chk`), which makes their
`_query` hash check fail on Linux and GraphBLAS silently falls back to slow
generic kernels.
