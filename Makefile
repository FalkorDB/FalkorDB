...
ifneq ($(BUILD),0)
	...
	RUSTFLAGS+=' -L$(RAX_BINDIR) -lrax -L$(LIBXXHASH_BINDIR) -lxxhash -L$(LIBCYPHER_PARSER_BINDIR)/lib/src/.libs -lstatic=cypher-parser -L$(UTF8PROC_BINDIR) -lutf8proc -L$(ONIGURUMA_BINDIR) -lonig -L$(REDISEARCH_BINROOT)/search-static -lredisearch-static -L$(VECSIM_BINDIR) -lVectorSimilarity -L$(VECSIM_BINDIR)/spaces -lVectorSimilaritySpaces -lVectorSimilaritySpaces_avx -lVectorSimilaritySpaces_avx512 -lVectorSimilaritySpaces_avx512dq -lVectorSimilaritySpaces_no_optimization -lVectorSimilaritySpaces_sse -L$(BINROOT)/src -lfalkordb_static -L$(GRAPHBLAS_BINDIR) -lgraphblas -L/usr/lib/llvm-17/lib/ -lomp -lstdc++ -lssl -lcrypto' cargo test --lib
