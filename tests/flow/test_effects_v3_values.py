"""Effects v3 -- a null that must remove the property rather than store it.

The `SIValue` tags themselves round-trip in `graph/src/effects/v3/value.rs`
(`value_roundtrips_every_encodable_variant`), which is where a codec claim
belongs; this file is about the replica-side apply path.
