...
class testOptionalFlow(FlowTestsBase):
    ...
    def test25_optional_batch_traversal(self):
        ...
        plan = str(self.graph.explain(query))
        self.env.assertContains("Optional Conditional Traverse | (a)->(b)", plan)
        self.env.assertContains("Optional Conditional Traverse | (b)->(c)", plan)
    ...
    # a WHERE clause following a WITH after an OPTIONAL MATCH must not be
    # pushed below the Optional operation, otherwise the all-NULL row
    # synthesized by the empty optional match bypasses the filter entirely
    # see https://github.com/FalkorDB/FalkorDB/issues/2508
    def test28_filter_after_with_on_empty_optional_match(self):
        # use a fresh graph
