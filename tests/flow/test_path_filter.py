...
class testPathFilter(FlowTestsBase):
    ...
    def test04_test_level_1_nesting_logical_operators_over_path_and_property_filters(
        ...
        query_info = QueryInfo(
            query=query,
            description="Tests AND condition with simple filter and negated path filter",
            expected_result=expected_results,
        )
        self._assert_resultset_and_expected_mutually_included(result_set, query_info)
    ...
    def test07_test_edge_filters(self):
        ...
        edge01 = Edge(
            src_node=node0, dest_node=node1, relation="R", properties={"x": 1}
        )
    ...
    def test08_indexed_child_stream_resolution(self):
        ...
        query = (
            "MATCH (n:L) WHERE (:L)<-[]-(n)<-[]-(:L {x: 'a'}) AND n.x = 'b' RETURN n.x"
        )
        result_set = self.graph.query(query)
        expected_results = [["b"]]
    ...
    def test10_verify_apply_results(self):
        ...
        edge12 = Edge(src_node=node1, dest_node=node2, relation="R")
        self.graph.query(
            f"CREATE {node0}, {node1}, {node2}, {edge01}, {edge02}, {edge12}"
        )
    ...
    def test14_path_and_predicate_filters(self):
        ...
        self.env.assertTrue(
            re.search("Semi Apply\s+Filter\s+Node By Label Scan", plan_1)
        )
        result_set = self.graph.query(query)
        expected_result = [["a"]]
        self.env.assertEquals(result_set.result_set, expected_result)
    ...
    def test16_xor_with_path_filter_should_error(self):
        # Test that XOR with path filters returns a proper error instead of crashing
        # This tests the fix for issue #1466
        node0 = Node(alias="n0", node_id=0, labels="L")
        node1 = Node(alias="n1", node_id=1, labels="L", properties={"x": 1})
        edge01 = Edge(src_node=node0, dest_node=node1, relation="R")
        self.graph.query(f"CREATE {node0}, {node1}, {edge01}")

        # Test 1: XOR with anonymous pattern (original issue)
        try:
        ...
        # Test 2: Simple XOR with path filter
        try:
        ...
        # Test 3: XOR in OPTIONAL MATCH (from issue comment)
        try:
        ...
        # Test 4: XOR in WITH clause (from issue comment)
        try:
        ...
        # Test 5: Verify AND and OR still work with path filters
        query = "MATCH (n:L) WHERE (n)-[:R]->(:L) AND n.x IS NULL RETURN n"
        result_set = self.graph.query(query)
        expected_results = [[node0]]
        query_info = QueryInfo(
            query=query,
            description="Tests AND with path filter still works",
            expected_result=expected_results,
        )
        self._assert_resultset_equals_expected(result_set, query_info)
        ...
        query_info = QueryInfo(
            query=query,
            description="Tests OR with path filter still works",
            expected_result=expected_results,
        )
        self._assert_resultset_and_expected_mutually_included(result_set, query_info)
