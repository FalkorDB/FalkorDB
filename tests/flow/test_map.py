...
class testMap(FlowTestsBase):
    ...
    def test09_merge_map(self):
        ...
        self.env.assertEqual(actual_result.result_set[0][0], {'name': 'John', 'age': 40, 'city': 'New York'})
    ...
    def test10_map_merge_null(self):
        ...
        self.env.assertEqual(res[0][0], {'name': 'John', 'age': 30, 'x': [{'z':1}]})
        ...
        res = self.graph.query(q).result_set
        self.env.assertEqual(len(res), 1)
        self.env.assertEqual(res[0][1], {'name': 'John', 'age': 30, 'x': [{'z':1}]})
