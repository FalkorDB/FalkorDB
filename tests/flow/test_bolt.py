...
class testBolt():
    ...
    def test33_map_param_set_update(self):
        """Test that a map passed as a Bolt parameter can be used in SET +=.
        This is the core scenario from the issue: passing a map as a parameter
        for bulk property updates using SET node += $mapParam."""
        with self.bolt_con.session() as session:
            # create a node
            session.run("CREATE (:TestNode {id: 'abc'})").consume()
            # merge and set properties via map parameter
            props = {"title": "Test", "uri": "http://example.com"}
            result = session.run(
                "MERGE (e:TestNode {id: $id}) SET e += $properties RETURN e",
                {"id": "abc", "properties": props})
            record = result.single()
            node = record[0]
            self.env.assertEquals(node["id"], "abc")
            self.env.assertEquals(node["title"], "Test")
            self.env.assertEquals(node["uri"], "http://example.com")
            # cleanup
            session.run("MATCH (n:TestNode) DELETE n").consume()

    def test34_map_param_string_special_chars(self):
        ...
        with self.bolt_con.session() as session:
            ...
            result = session.run("RETURN $v", {"v": m})
            record = result.single()
            self.env.assertEquals(record[0], m)

    def test35_map_param_non_identifier_keys(self):
        """Test that map keys with non-identifier characters (hyphens, spaces,
        dots) are properly backtick-quoted in the CYPHER parameter format."""
        with self.bolt_con.session() as session:
            # key with a hyphen
            m = {"my-key": "value"}
            result = session.run("RETURN $v", {"v": m})
    ...
    def test36_map_param_multiple_params(self):
        """Test that multiple parameters work correctly when one is a map.
        This tests the case that caused the original parse error: with both a
        scalar and a map parameter, the CYPHER prefix must be fully stripped."""
        with self.bolt_con.session() as session:
            props = {"title": "Test", "uri": "http://example.com"}
            result = session.run(
                ...
                {"id": "abc", "properties": props})
            record = result.single()
            self.env.assertEquals(record[0], "abc")
            self.env.assertEquals(record[1], props)
