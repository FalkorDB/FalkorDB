...
void test_effectsV3Wire_corpusMountStatus(void) {
	...
	if(fixtures && !harness) {
		...
		printf("\n"
			...
			"   %s\n"
			"\n"
			"   The fixture-driven harness -- corpus integrity, the byte\n"
			"   round trip, and the truncation sweep -- is NOT PART OF THIS\n"
			"   RUN. Those tests are not failing; they do not exist, because\n"
			"   tests/unit/CMakeLists.txt globs test_*.c at configure time\n"
			"   and their sources are on the aux branch.\n"
			"\n"
			"   To mount it:\n"
			"     %s\n"
			"   then copy its tests/unit sources into tests/unit/ and re-run\n"
			"   cmake, so the glob picks them up.\n"
			"\n"
