...
void Register_StringFuncs() {
	...
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("ltrim", AR_LTRIM, 1, 1, types, ret_type, false, true, "returns the original string with leading whitespace removed");
	AR_RegFunc(func_desc);
	...
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("rtrim", AR_RTRIM, 1, 1, types, ret_type, false, true, "returns the original string with trailing whitespace removed");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, (T_STRING | T_ARRAY | T_NULL));
	ret_type = T_STRING | T_ARRAY | T_NULL;
	func_desc = AR_FuncDescNew("reverse", AR_REVERSE, 1, 1, types, ret_type, false, true, "returns a string in which the order of all characters in the original string have been reversed");
	AR_RegFunc(func_desc);
	...
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("string.join", AR_JOIN, 1, 2, types, ret_type, false, true, "given a list of strings and an optional delimiter, return a concatenation of all the strings using the given delimiter");
	AR_RegFunc(func_desc);
	...
	func_desc = AR_FuncDescNew("string.matchRegEx", AR_MATCHREGEX, 2, 2, types, ret_type, false, true, "given a string and a regular expression, return an array of all matches and matching regions");
	AR_RegFunc(func_desc);
	...
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("string.replaceRegEx", AR_REPLACEREGEX, 2, 3, types, ret_type, false, true, "given a string and a regular expression, return a string after replacing each regex match with a given replacement");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("tolower", AR_TOLOWER, 1, 1, types, ret_type, false, true, "returns the original string in lowercase");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("toupper", AR_TOUPPER, 1, 1, types, ret_type, false, true, "returns the original string in uppercase");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, STRINGABLE | T_NULL);
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("tostring", AR_TOSTRING, 1, 1, types, ret_type, false, true, "converts an integer, float or boolean value to a string");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, SI_ALL);
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("tostringornull", AR_TOSTRING, 1, 1, types, ret_type, false, true, "converts an integer, float or boolean value to a string");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, SI_ALL);
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("tojson", AR_TOJSON, 1, 1, types, ret_type, false, true, "returns a JSON string representation of a map value");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("trim", AR_TRIM, 1, 1, types, ret_type, false, true, "returns the original string with leading and trailing whitespace removed");
	AR_RegFunc(func_desc);
	...
	ret_type = T_STRING;
	func_desc = AR_FuncDescNew("randomuuid", AR_RANDOMUUID, 0, 0, types, ret_type, false, false, "returns a random UUID");
	AR_RegFunc(func_desc);
	...
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_STRING | T_NULL;
	func_desc = AR_FuncDescNew("replace", AR_REPLACE, 3, 3, types, ret_type, false, true, "returns a string in which all occurrences of a specified string in the original string have been replaced by ANOTHER (specified) string");
	AR_RegFunc(func_desc);
	...
	array_append(types, (T_STRING | T_NULL));
	ret_type = T_ARRAY | T_NULL;
	func_desc = AR_FuncDescNew("split", AR_SPLIT, 2, 2, types, ret_type, false, true, "returns a list of strings resulting from the splitting of the original string around matches of the given delimiter");
	AR_RegFunc(func_desc);
}
