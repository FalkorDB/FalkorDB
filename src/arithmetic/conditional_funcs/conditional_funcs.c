...
void Register_ConditionalFuncs() {
	...
	func_desc = AR_FuncDescNew("case", AR_CASEWHEN, 2, VAR_ARG_LEN, types, ret_type, true, true, "case when");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, SI_ALL);
	func_desc = AR_FuncDescNew("coalesce", AR_COALESCE, 1, VAR_ARG_LEN, types, ret_type, false, true, "coalesce");
	AR_RegFunc(func_desc);

	types = array_new(SIType, 1);
	array_append(types, SI_ALL);
	func_desc = AR_FuncDescNew("distinct", AR_DISTINCT, 1, 1, types, ret_type, true, false, "distinct");
