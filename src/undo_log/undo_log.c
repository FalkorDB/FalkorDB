...
(
	...
	for (int i = seq_start ; i > seq_end ; --i) {
		...
		if (update_op->entity_type == GETYPE_NODE) {
			// regain attribute ownership
			AttributeSet_TransferOwnership (*update_op->n.attributes,
					update_op->set) ;
			...
			*update_op->n.attributes = update_op->set ;
			...
			// regain attribute ownership
			AttributeSet_TransferOwnership (*update_op->e.attributes,
					update_op->set) ;
...
// rolls back a sequence of UNDO_SET_LABELS operations by stripping
// the added labels from their original nodes
// processes ops in the range (seq_end, seq_start], inclusive of seq_start
static void _UndoLog_Rollback_Set_Labels
(
	QueryCtx *ctx,  // query context holding the undo log
	int seq_start,  // index of the first op in the sequence (inclusive)
	int seq_end     // index of the last  op in the sequence (exclusive)
) {
	ASSERT (ctx != NULL) ;
...
// the stripped labels to their original nodes
// processes ops in the range (seq_end, seq_start], inclusive of seq_start
...
(
	...
	ATTRIBUTE_SET_MARK_READONLY (node->attributes) ;
...
(
	...
	ATTRIBUTE_SET_MARK_READONLY (edge->attributes) ;
...
(
	...
	switch (op->type) {
		...
		case UNDO_SET_LABELS:
		case UNDO_REMOVE_LABELS:
			GrB_OK (GrB_free (&op->labels_op.nodes)) ;
			break;
