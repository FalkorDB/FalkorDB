...
QGNode **BFS(QGNode *s, int *level) {
	...
	QGNode **next = arr_new(QGNode *, 0);     // Nodes to explore next.
	QGNode **current = arr_new(QGNode *, 1);  // Nodes currently explored.

	arr_append(current, s);

	// As long as we've yet to reach required level and there are nodes to process.
	while(current_level < *level && arr_len(current)) {
		// As long as there are nodes in the frontier.
		for(int i = 0; i < arr_len(current); i++) {
			QGNode *n = current[i];
		...
		if(arr_len(next) == 0 && *level == BFS_LOWEST_LEVEL) {
			*level = current_level;
			break;
		}
