...
(
	...
	clone->record_map = raxNew();

	if(template->ast_segment) {
...
(
	...
	OpBase *children[template_current->childCount];
	for(uint i = 0; i < template_current->childCount; i++) {
		// recursively visit and clone the op's children
		children[i] = _CloneOpTree(template_current->children[i], old_to_new);
	}
	...
	plan_segment->root = clone_current;

	for(uint i = 0; i < template_current->childCount; i++) {
		OpBase_AddChild(clone_current, children[i]);
	}
...
// of ops, when an op is encountered that was constructed as part of a different
// ExecutionPlan segment, that segment and its internal members
// (FilterTree, record mapping, query graphs, and AST segment) are also cloned
