...
static OpBase *SemiApplyClone(ExecutionPlan *plan, const OpBase *opBase);
...
(
	...
	// the op bounded branch and match branch are set to be
	// the first and second child, respectively during the operation building
	// procedure at execution_plan_reduce_to_apply.c
...
// set it as an argument for the op match branch and consumes a record from
// the match branch if there is a record from the match branch
...
(
	...
	while(true) {
		...
		if(rhs_record) {
			...
			// right stream returned NULL
			// return left handside record
			Record r = op->r;
			op->r = NULL;   // Null to avoid double free
			return r;
		}
