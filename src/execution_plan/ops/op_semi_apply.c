...
(
	...
	// the op bounded branch and match branch are set to be the first and
	...
	ASSERT(op->bound_branch != NULL);
	ASSERT(op->match_branch != NULL);
...
(
	...
	while(true) {
		...
		Record rhs_record = OpBase_Consume(op->match_branch);
