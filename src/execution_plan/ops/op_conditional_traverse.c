...
static OpBase *CondTraverseClone(ExecutionPlan *plan, const OpBase *opBase);
...
(
	...
	// we can return early otherwise
	// try to get a new pair of source and destination nodes
	if(op->r         != NULL  &&
	   op->edge_ctx  != NULL  &&
