...
static OpBase *OpApplyMultiplexerClone(ExecutionPlan *plan, const OpBase *opBase);
...
// in this init function, the children 1..n are sorted such that
// branches which are filter ops will be placed at
// the begining of the children array
// and branches which are apply ops will be placed at the end
// of the children array since filters are easier to evalute
...
(
	...
	while(true) {
		...
		// all branches returned record => all filters are satisfied
		// by the bounded record
		Record r = op->r;
		op->r = NULL;   // null to avoid double free
		return r;
	}
