...
// sorts the multiplexer children
// Apply operations to the very end (rightmost),
// filter operations to the beginning (leftmost)
// the filter operations evaluted faster then the apply operations so we want
// to evaluate them first
static void _OpApplyMultiplexer_SortChildren
(
	OpBase *op
) {
	// the 0 child is the bounded branch of this operation, which consumes the
	// record and is set on execution_plan_reduce_to_apply.c
...
// are filter ops will be placed at
// the begining of the children array, and branches which are apply ops will be
// placed at the end of the children array, since filters are easier to evalute.
...
(
	...
	while(true) {
		...
		// all branches returned record => all filters are satisfied by the
		// bounded record
		Record r = op->r;
		op->r = NULL;   // null to avoid double free
		return r;
