...
(
	...
	for(uint i = 0; i < filter_count; i ++) {
		...
		OpBase_AddChild((OpBase *)filter_op, new_cp);
		...
		for(uint j = 0; j < solving_branch_count; j++) {
			...
			ExecutionPlan_RemoveBranch(solving_branch);
			OpBase_AddChild(new_cp, solving_branch);
		}
		array_free(solving_branches);
		...
		if(cp->childCount == 0) {
			...
			OpBase_AddChild(cp, (OpBase *)filter_op);
		}
	}
