...
static void _removeRedundantTraversal(ExecutionPlan *plan, OpCondTraverse *traverse) {
	...
	if(src_alias && dest_alias && AlgebraicExpression_OperandCount(ae) == 1 &&
	   !strcmp(src_alias, dest_alias)) {
...
void reduceTraversal(ExecutionPlan *plan) {
	...
	for(uint i = 0; i < traversals_count; i++) {
		...
		const char* src_alias = AlgebraicExpression_Src(ae);
		const char* dest_alias = AlgebraicExpression_Dest(ae);
		if(src_alias && dest_alias && !strcmp(src_alias, dest_alias) &&
		...
		if(op->type == OPType_CONDITIONAL_TRAVERSE) {
			...
			OpBase *t;
			const char *ae_src_alias = AlgebraicExpression_Src(ae);
			QGNode *src = ae_src_alias ? QueryGraph_GetNodeByAlias(traverse_plan->query_graph, ae_src_alias) : NULL;
			...
			const char *ae_dest_alias = AlgebraicExpression_Dest(ae);
			QGNode *dest = ae_dest_alias ? QueryGraph_GetNodeByAlias(traverse_plan->query_graph, ae_dest_alias) : NULL;
