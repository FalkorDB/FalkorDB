	...
	NodeByLabelScan *scan)
{
	...
	OPType parent_type = OpBase_Type(parent);
	if (parent_type != OPType_CONDITIONAL_TRAVERSE &&
		parent_type != OPType_EXPAND_INTO &&
		parent_type != OPType_CONDITIONAL_VAR_LEN_TRAVERSE &&
		parent_type != OPType_OPTIONAL_CONDITIONAL_TRAVERSE &&
		parent_type != OPType_CONDITIONAL_VAR_LEN_TRAVERSE_EXPAND_INTO)
	{
		return false;
	}
