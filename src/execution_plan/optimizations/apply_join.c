...
// given an expression node from a filter tree
// returns the stream number that fully resolves the expression's references
...
// reutnrs stream index if found
// NOT_RESOLVED if non of the stream resolve the expression
...
// this function builds a Hash Join operation given its left and right branches
// and join criteria
...
(
	...
	// our current heuristic for this is to prefer a stream which contains
	// a filter operation
	...
	OpBase_AddChild(value_hash_join, left_branch);
	OpBase_AddChild(value_hash_join, right_branch);
...
(
	...
	for(uint i = 0; i < filter_count; i ++) {
		...
		// each filter being considered here tests for equality between its
		// left and right values the Cartesian Product can be replaced if
		// both sides of the filter can be fully and separately resolved by
		// exactly two child streams
		...
		// this filter is solved by a single cartesian product child and needs
		// to be propagated up
		...
		ExecutionPlan_RemoveBranch(right_branch);
		ExecutionPlan_RemoveBranch(left_branch);
		// build hash join op
		OpBase *value_hash_join = _build_hash_join_op(cp->plan, left_branch,
				right_branch, lhs, rhs);
		...
		if(cp->childCount == 0) {
			...
			// the new operation since the original cartesian product is
			// no longer a valid operation and there might be additional filters
			// which are applicable to re position after the optimization
			// is done the following code tries to propagate up
			// the remaining filters and finish the loop
			i++;
			...
			OpBase_AddChild(cp, value_hash_join);
