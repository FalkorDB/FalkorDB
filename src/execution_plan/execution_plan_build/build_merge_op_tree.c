...
(
	...
	// we must ensure that all of our created entities are unique
	// consider:
	// UNWIND [1, 1] AS x MERGE ({val: x})
	// exactly one node should be created in the UNWIND...MERGE query
	OpBase *merge_create = NewMergeCreateOp (plan, merge_ctx->nodes_to_merge,
			merge_ctx->edges_to_merge) ;

	// add MergeCreate op to stream
	ExecutionPlan_AddOp (plan->root, merge_create) ;

	// if we have bound variables, push an Argument tap beneath the Create op
	if (arguments) {
		OpBase *create_argument = NewArgumentOp (plan, arguments) ;
		// add Argument op to stream
		ExecutionPlan_AddOp (merge_create, create_argument) ;
	}
}
