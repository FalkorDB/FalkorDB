...
// strings enclosed in the parentheses of a CALL clause represent the arguments
// to the procedure _BuildCallArguments creates a string array holding all of
// these arguments
static AR_ExpNode **_BuildCallArguments
(
	...
	for(uint i = 0; i < arg_count; i ++) {
		const cypher_astnode_t *exp =
			cypher_ast_call_get_argument(call_clause, i);
...
// _BuildCallProjections creates an array of expression nodes to populate
// a Project operation with all strings in the YIELD block of a CALL clause
// are represented
// or the procedure-registered outputs if the YIELD block is missing
static AR_ExpNode **_BuildCallProjections
(
	const cypher_astnode_t *call_clause
) {
	...
	for(uint i = 0; i < yield_count; i++) {
		...
		if(alias_node) {
			// the projection either has an alias (AS)
			// is a function call, or is a property specification (e.name)
	...
	// if the procedure call is missing its yield part
	// include procedure outputs
	if(yield_count == 0) {
		const char *proc_name =
			cypher_ast_proc_name_get_value(
					cypher_ast_call_get_proc_name(call_clause));
