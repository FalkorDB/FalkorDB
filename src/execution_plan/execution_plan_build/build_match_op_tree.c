...
(
	...
	for(uint i = 0; i < n; i++) {
		...
		if(bounded) {
			...
			ASSERT(tail != NULL);
			OpBase_AddChild(tail, arg);
		}
	}

	ASSERT(OpBase_ChildCount(cp) > 1);
	...
	if(bounded) {
		...
		OpBase_AddChild(apply, cp);
	} else {
		ExecutionPlan_UpdateRoot(plan, cp);
	}
}
...
(
	...
	OpBase_AddChild(optional, match_stream);
	...
	if(plan->root) {
		...
		OpBase_AddChild(apply_op, optional);
	} else {
