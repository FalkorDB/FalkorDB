...
(
	...
	ASSERT(a >= 0);
	ASSERT(b >= 0);
	ASSERT(a < op->childCount);
	ASSERT(b < op->childCount);
...
(
	...
	// in case of Apply operations, we need to append the new argument branch
	// and set it as the first child
	OpBase_AddChild(op, arg);
...
(
	...
	OpBase_AddChild(op_semi_apply, match_branch);
...
(
	...
	if(filter_root->t == FT_N_COND && FilterTree_ContainsFunc(filter_root, "path_filter", &node)) {
		...
		OpBase_AddChild(apply_multiplexer, lhs);
		OpBase_AddChild(apply_multiplexer, rhs);
...
(
	...
	ExecutionPlan_BoundVariables((OpBase *)filter, bound_vars,
		((OpBase *)filter)->plan);
	// collect the variable names from bound_vars to populate the Argument ops
	// we will build
