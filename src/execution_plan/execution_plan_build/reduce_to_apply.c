...
(
	...
	const cypher_astnode_t *path =
		expression->op.children[0]->operand.constant.ptrval;
...
(
	...
	if(filter_root->t == FT_N_EXP &&
	   FilterTree_ContainsFunc(filter_root, "path_filter", &node)) {
...
(
	...
	ExecutionPlan_BoundVariables((OpBase *)filter, bound_vars,
		((OpBase *)filter)->plan);
	// collect the variable names to populate an Argument op
