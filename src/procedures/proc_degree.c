...
typedef struct {
	...
	SIValue output [3];     // array with up to two entries [node, degree]
...
(
	...
	// TODO: should only be calculated if caller requests degree
	//--------------------------------------------------------------------------
	...
	//--------------------------------------------------------------------------
	if(deg_ctx.attribute != ATTRIBUTE_ID_NONE){
		if(rel_types != NULL) {
			for(unsigned short i = 0; i < n_rels; i++) {
				R = Graph_GetRelationMatrix(g, rel_types[i], false);
				int opt = Graph_RelationshipContainsMultiEdge(g, rel_types[i])?
...
(
	...
	if(pdata->yield_weight) {
		// TODO: possibly have two iterators?
		double weight = 0;
		GrB_Info info = GrB_Vector_extractElement_FP64(&weight, pdata->w_degree, id);
		ASSERT(info == GrB_SUCCESS);
