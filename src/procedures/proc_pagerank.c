...
(
	...
	if(label) {
		...
		Delta_Matrix_export(&l, Graph_GetLabelMatrix(g, s->id), GrB_BOOL);
	}
	...
	if(relation) {
		...
		if(!s) return PROCEDURE_OK;
		Delta_Matrix_export(&r, Graph_GetRelationMatrix(g, s->id, false), GrB_BOOL);
		...
		Delta_Matrix_export(&r, Graph_GetAdjacencyMatrix(g, false), GrB_BOOL);
	}
