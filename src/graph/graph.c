...
static void _CreateRWLock(Graph *g) {
...
void Graph_ReleaseLock(Graph *g) {
  ...
  // no harm done, if we set `_writelocked` to false after unlocking it is
  // possible for a reader thread to be considered as writer, performing illegal
  // access to underline matrices, consider a context switch after unlocking
  // `_rwlock` but before setting `_writelocked` to false
...
static inline AttributeSet *_Graph_GetEntity(const DataBlock *entities,
                                             EntityID id) {
...
void _MatrixSynchronize(const Graph *g,  // graph the matrix is related to
...
void _MatrixResizeToCapacity(const Graph *g,  // graph the matrix is related to
                             Delta_Matrix M,  // matrix to synchronize
                             GrB_Index nrows, // # of rows for the resize
                             GrB_Index ncols  // # of columns for the resize
) {
...
void _MatrixNOP(const Graph *g,  // graph the matrix is related to
                Delta_Matrix M,  // matrix to synchronize
                GrB_Index nrows, // # of rows for the resize
                GrB_Index ncols  // # of columns for the resize
) {
...
MATRIX_POLICY Graph_SetMatrixPolicy(Graph *g, MATRIX_POLICY policy) {
  ...
  case SYNC_POLICY_NOP:
    // Used when deleting or freeing a graph; forces no matrix updates or
    // resizes.
...
void Graph_ApplyAllPending(Graph *g, bool force_flush) {
  ASSERT(g != NULL);
...
// taking a snapshot
void Graph_LockAllMatrices(Graph *g // graph to lock
) {
  ASSERT(g != NULL);
...
void Graph_UnlockAllMatrices(Graph *g // graph to unlock
) {
  ASSERT(g != NULL);
...
bool Graph_Pending(const Graph *g) {
  ASSERT(g != NULL);
...
// create a new graph
Graph *Graph_New(size_t node_cap, // allocation size for node datablocks and
                                  // matrix dimensions
  ...
  Delta_Matrix_new(&g->adjacency_matrix, GrB_UINT64, n, n, true);
...
static void
_GetOutgoingNodeEdges(const Graph *g, // graph to collect edges from
...
static void
_GetIncomingNodeEdges(const Graph *g,       // graph to collect edges from
                      const Node *n,        // either source or destination node
                      RelationID r,         // relationship type
...
static void _Graph_FreeRelationMatrices(const Graph *g) {
...
LabelID Graph_AddLabel(Graph *g) {
  ASSERT(g != NULL);
...
void Graph_RemoveLabel(Graph *g, LabelID label_id) {
  ASSERT(g != NULL);
...
RelationID Graph_AddRelationType(Graph *g) {
  ASSERT(g);
...
void Graph_RemoveRelation(Graph *g, RelationID relation_id) {
  ASSERT(g != NULL);
...
void Graph_AllocateNodes(Graph *g, // graph for which nodes will be added
...
void Graph_AllocateEdges(Graph *g, // graph for which nodes will be added
...
void Graph_ResetReservedNode(Graph *g // graph
) {
  ASSERT(g != NULL);
...
// reserve a node
Node Graph_ReserveNode(Graph *g // graph for which nodes will be added
) {
  ASSERT(g != NULL);
...
void Graph_CreateNode(Graph *g,        // graph
...
void Graph_LabelNode(Graph *g,      // graph to operate on
...
bool Graph_IsNodeLabeled(Graph *g,  // graph to operate on
...
void Graph_RemoveNodeLabels(Graph *g,      // graph to operate against
                            NodeID id,     // node ID to update
...
void Graph_CreateEdge(Graph *g,     // graph on which to operate
...
static int _edge_src_dest_cmp(const void *a, const void *b) {
...
void Graph_ClearConnections(Graph *g,    // graph to update
                            Edge *edges, // edges to clear
                            uint64_t n   // number of edges
);

// deletes edges from the graph
void Graph_DeleteEdges(Graph *g,    // graph to delete edges from
...
inline bool Graph_EntityIsDeleted(const GraphEntity *e) {
...
void Graph_DeletedNodes(const Graph *g, // graph
...
uint64_t Graph_NodeCap(const Graph *g) { return g->nodes->itemCap; }

// returns number of nodes in the graph
uint64_t Graph_NodeCount(const Graph *g) {
  ASSERT(g);
...
int Graph_RelationTypeCount(const Graph *g) { return array_len(g->relations); }

// returns number of different node types
int Graph_LabelTypeCount(const Graph *g) { return array_len(g->labels); }

// returns true if relationship matrix 'r' contains multi-edge entries,
// false otherwise
bool Graph_RelationshipContainsMultiEdge(const Graph *g, RelationID r) {
...
bool Graph_GetNode(const Graph *g, NodeID id, Node *n) {
  ASSERT(g != NULL);
  ASSERT(n != NULL);
...
bool Graph_GetEdge(const Graph *g, EdgeID id, Edge *e) {
  ASSERT(g != NULL);
  ASSERT(e != NULL);
...
void Graph_GetNodeEdges(const Graph *g,     // graph to collect edges from
                        const Node *n,      // either source or destination node
  ...
  bool outgoing =
      (dir == GRAPH_EDGE_DIR_OUTGOING || dir == GRAPH_EDGE_DIR_BOTH);

  bool incoming =
      (dir == GRAPH_EDGE_DIR_INCOMING || dir == GRAPH_EDGE_DIR_BOTH);
...
uint64_t Graph_GetNodeDegree(const Graph *g,     // graph to inquery
...
uint Graph_GetNodeLabels(const Graph *g,  // graph the node belongs to
...
Delta_Matrix Graph_GetAdjacencyMatrix(const Graph *g, bool transposed) {
  return Graph_GetRelationMatrix(g, GRAPH_NO_RELATION, transposed);
}

// retrieves a label matrix
// matrix is resized if its size doesn't match graph's node count
Delta_Matrix Graph_GetLabelMatrix(const Graph *g, LabelID label_idx) {
  ASSERT(g != NULL);
...
// matrix is resized if its size doesn't match graph's node count.
Delta_Matrix Graph_GetNodeLabelMatrix(const Graph *g) {
  ASSERT(g != NULL);
...
Delta_Matrix Graph_GetZeroMatrix(const Graph *g) {
...
static void _Graph_Free(Graph *g, bool is_full_graph) {
  ASSERT(g);
  // free matrices
  AttributeSet *set;
...
void Graph_PartialFree(Graph *g) { _Graph_Free(g, false); }

// free graph
void Graph_Free(Graph *g) { _Graph_Free(g, true); }
