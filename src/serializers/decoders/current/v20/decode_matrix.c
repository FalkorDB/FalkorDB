...
(
	...
	GrB_OK (GxB_Type_from_name (&t, t_name)) ;
	rm_free (t_name) ;

	// load vector
	GrB_OK (GrB_Vector_new (v, t, 0)) ;
	GrB_OK (GxB_Vector_load (*v, &arr, t, n_entries, n_bytes, handling, NULL)) ;
}
...
(
	...
	GrB_OK (GrB_Matrix_new (&A, GrB_BOOL, 0, 0)) ;  // matrix type doesn't matter
	...
	GrB_OK (GxB_Container_free (&container)) ; // does several O(1)-sized free’s
