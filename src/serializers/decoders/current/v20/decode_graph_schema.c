...
(
	...
	// graph-level CCH path indices are no longer part of the (per-vkey) schema
	// section -- they are encoded once, with their full hierarchy, in a dedicated
	// payload state (see RdbLoadCCH_v20 / ENCODE_STATE_CCH_INDICES)
}


// load the graph-level CCH path indices: each index is read whole (identity +
// full hierarchy) and registered. encoded once, in its own payload state
// (ENCODE_STATE_CCH_INDICES), so this runs exactly once per decode -- no
// per-vkey de-duplication and no post-load rebuild
void RdbLoadCCH_v20
(
	SerializerIO rdb,
	GraphContext *gc
