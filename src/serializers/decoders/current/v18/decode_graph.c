...
(
	...
	*gc = NULL ;
	...
	// Graph name
	char *graph_name ;
	if (!SerializerIO_TryReadBuffer (io,(void**) &graph_name, NULL)) {
		return false ;
	}
	...
	if (first_vkey == true) {
		...
		for(uint i = 0; i < relation_count; i++) {
			...
			array_append (_gc->decoding_context->multi_edge, multi_edge[i]) ;
		}

		GraphDecodeContext_SetKeyCount (_gc->decoding_context, key_number) ;
	}

	// decode graph schemas
	if (!RdbLoadGraphSchema_v18 (io, _gc, !first_vkey)) {
		return false ;
	}

	// save decode statistics for later progess reporting
	// e.g. "Decoded 20000/4500000 nodes"
	_gc->decoding_context->node_count         = node_count ;
	_gc->decoding_context->edge_count         = edge_count ;
	_gc->decoding_context->deleted_node_count = deleted_node_count ;
	_gc->decoding_context->deleted_edge_count = deleted_edge_count ;
...
(
	...
	*payloads = array_new (PayloadInfo, payloads_count) ;
	...
	for (uint i = 0; i < payloads_count; i++) {
		...
		TRY_READ (io, payload_info.entities_count) ;
	...
	for (uint i = 0; i < payloads_count; i++) {
		...
		switch (payload.state) {
			case ENCODE_STATE_NODES:
				...
				RdbLoadNodes_v18 (io, g, payload.entities_count) ;
			...
			case ENCODE_STATE_DELETED_NODES:
				RdbLoadDeletedNodes_v18 (io, g, payload.entities_count) ;
			...
			case ENCODE_STATE_EDGES:
				Graph_SetMatrixPolicy (g, SYNC_POLICY_NOP) ;
				RdbLoadEdges_v18 (io, g, payload.entities_count) ;
			...
			case ENCODE_STATE_DELETED_EDGES:
				RdbLoadDeletedEdges_v18 (io, g, payload.entities_count) ;
			...
			case ENCODE_STATE_RELATION_MATRICES:
				...
				LoadRelationMatrices_v18 (io, gc) ;
				break;
