...
(
	...
	switch(t) {
		...
		case T_STRING: {
				...
				char *x ;
				if (!SerializerIO_TryReadBuffer (io, (void**)&x, NULL)) {
					return false ;
				}
...
(
	...
	char *elements ;
	if (!SerializerIO_TryReadBuffer (io, (void**)&elements, NULL)) {
		return false ;
	}
...
(
	...
	if (!SerializerIO_TryReadBuffer (io, (void**)&deleted_nodes_list, &n)) {
		return false ;
	}
...
(
	...
	if (!SerializerIO_TryReadBuffer (io, (void**)&deleted_edges_list, &n)) {
		return false ;
	}
